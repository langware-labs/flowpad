...
class BlobManager {
  ...
  async checkLatest() {
    ...
    if (process.env.FLOWPAD_BLOB_LOCAL) return process.env.FLOWPAD_BLOB_VERSION || '0.0.0-local';
    try {
  ...
  async _installVersion(version) {
    ...
    const local = process.env.FLOWPAD_BLOB_LOCAL;
    if (local) {
      ...
      const url = `https://github.com/${RELEASE_REPO}/releases/download/v${version}/flowpad-backend-${version}-${PLATFORM_TAG}.zip`;
  ...
  async _buildEnv() {
    ...
    const env = {
      ...
      LOCAL_SERVER_PORT: BACKEND_PORT,

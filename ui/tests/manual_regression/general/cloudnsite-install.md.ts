...
const REQUIRED_INPUTS = [
  ...
  'CLOUDNSITE_E2E_EMAIL',
  'CLOUDNSITE_E2E_PASSWORD',
...
async function runInstall(
  ...
  const branchRow = page.getByRole('listitem').filter({
    has: page.getByText(config.targetBranch, { exact: true }),
  });
  ...
  return { workspace, nodeId, setup, consoleSecretLeak: () => consoleSecretLeak };
}
...
async function verifyWorkspace(config: Config, result: RunResult): Promise<void> {
...
async function cleanupWorkspace(context: BrowserContext, config: Config, nodeId: string): Promise<void> {
...
test.describe.serial('CloudNSite install link — live Hub and E2B release gate', () => {
  test.skip(!configured, `opt-in: set ${REQUIRED_INPUTS.join(', ')}`);

  let cleanup: { config: Config; nodeId: string } | null = null;

  test.afterEach(async ({ context }) => {
    if (!cleanup) return;
    await cleanupWorkspace(context, cleanup.config, cleanup.nodeId);
    cleanup = null;
  });
  ...
  test('clean install creates a review branch and opens the complete CloudNSite experience', async ({
    ...
    await verifyWorkspace(config, result);

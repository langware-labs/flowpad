...
# Corrected 2026-06-04: the wiki edge table is `links` with cols
# (src_type, src_id, target_raw, target_resolved_type, target_resolved_id, line).
# The raw wiki target name lives in `target_raw` — there is NO `target_name`
# column. DB path is per-instance: ~/.flow/instances/<instance>/flowpad.db.
# A whiteboard's wiki body is its WHITE_BOARD.md
...
- [bash] run: create source whiteboard — curl -s -X POST {API_URL}/api/v1/graph/whiteboard -H 'Content-Type: application/json' -d '{"name":"wiki-src-q2"}' ; capture asset_ref ; then materialize WHITE_BOARD.md by opening the editor and triggering one save
- navigate away from the editor (e.g. to /dock/home) to UNMOUNT it BEFORE editing WHITE_BOARD.md — a pending debounced autosave would otherwise read-modify-write the file after the append and clobber the [[link]]
...
- [bash] run: query the wiki graph — sqlite3 ~/.flow/instances/<instance>/flowpad.db "SELECT count(*) FROM links WHERE src_type='whiteboard' AND target_raw='wiki-target-q2';"

...
| VW-19 | Create another | Ask `Create another <asset type> based on this one and open it` | The new asset is created, `flow show` from the workspace's process pins it in the Display (it becomes a child only when opened from Display history), and the same chat remains mounted | Original remains in the child strip; no duplicate process or target-history key; reload restores the new child |
| VW-20 | Open related | Ask `Open the related <known asset>` | The existing related asset is pinned in the launching process's Display; reopened from history it lands as a child at the hosted URL naming that process | Correct viewer/type icon, one tab per target, chat and ordered child history stay live |

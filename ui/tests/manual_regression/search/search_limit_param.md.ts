...
test('Search limit parameter constrains result count', async ({ page }) => {
  ...
  const response = await page.request.get('http://localhost:9007/api/v1/search?q=test&limit=1');
  ...
  const response0 = await page.request.get('http://localhost:9007/api/v1/search?q=test&limit=0');
  expect(response0.status()).not.toBe(500);

  // Request with negative limit — should not crash
  const responseNeg = await page.request.get('http://localhost:9007/api/v1/search?q=test&limit=-1');

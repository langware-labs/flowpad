...
- validate `[data-testid="entity-chat-panel"]` is present
- validate the panel's nearest React fiber `EntityChatPanel` has prop `target` matching `<type>-<uuid>` (proves asset_ref → entity TypeId resolution via useEntityByPath)
- validate the chat textarea with placeholder "Ask about this doc…" is visible and NOT disabled (`taDisabled === false`)
- validate `[data-testid="entity-chat-status"]` is null/absent (no active process yet — empty state)
- validate `[data-testid="entity-chat-history"]` exists but is `disabled` (hasHistory === false on a fresh editor)
...
- validate within 5s: `[data-testid="entity-chat-status"]` becomes present (process was just lazy-created in `handleSend`)
...
- validate `[data-testid="entity-chat-history"]` becomes enabled (hasHistory === true now that one chat exists)

INVARIANT: spawn site is `EntityChatPanel.tsx:201` — `computeNode.createProcess({ targetTypeIdStr, outputFormat: 'stream-json' })` is invoked from `handleSend` only on first send, never on mount.
...
- validate `[data-testid="entity-chat-panel"]` re-mounts and the past process is auto-picked
- validate `[data-testid="entity-chat-status"]` text equals "DONE" or absent (terminal state) — NOT "Thinking" or "Started"
...
(see ui/src/components/entity-chat-panel/hooks/useDerivedWorkerStatus.ts). Before the fix,
hydrating history would replay assistant-START events and the indicator latched on
"Thinking" with no completion event to clear it. This test will fail if that filter is
removed.

test 4: target prop changes when switching between doc-type editors (asset_ref invalidation)
- open the plan URL; capture EntityChatPanel.target as `t_plan`
- navigate to the agent URL via the same browser tab (in-app navigation, not full reload)
- validate the new EntityChatPanel.target !== `t_plan` and starts with `agent-`
...
EntityChatPanel re-keys its picker state and `useProcessesForTarget(targetStr)` queries
fresh.

test 5 (BUG already in app — keep as known issue, don't fail the run):
- on first send to a fresh markdown doc, console emits "Cannot update a component
  (`EntityChatPanel`) while rendering a different component (`EntityChatPanel`)"
- this is a setState-in-render warning originating somewhere in the localProcess /
  resolvedMatchesLocal effect chain (EntityChatPanel.tsx:107-117).

 ...
 *
 * The active tab carries the connected-surface style (-mb-px border-b-transparent) — TabStrip.tsx.
...
test.describe('switch between sessions', () => {
  test('test 1: switching between two terminal tabs flips the active tab, no console errors', async ({ page }) => {
    ...
    const firstTab = page.locator(`[data-testid="tab-shell|${firstId}"]`);
    ...
    const secondId = ids.find((id) => id !== `shell|${firstId}`);
    expect(secondId).toBeTruthy();
    const secondTab = page.locator(`[data-testid="tab-${secondId}"]`);

    // Click the first tab → it becomes active (border-b-transparent).
    await firstTab.click();
    await expect(firstTab).toHaveClass(/border-b-transparent/, { timeout: 10_000 });

    // Click the second tab → it becomes active.
    await secondTab.click();
    await expect(secondTab).toHaveClass(/border-b-transparent/, { timeout: 10_000 });

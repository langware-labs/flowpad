...
async function indexType(
  ...
  // The index is single-flight per compute node: creating the fixture asset
  // kicks an auto-index, and a second POST while it runs is refused with
  // 409 "Job 'index' already running". Wait on the node's own activity signal
  // (null = idle) instead of racing it.
  // timeout 0 = poll until the TEST's own cap: the activity may be a full
  // background index (hundreds of rows), far beyond the 15s expect default.
  // This raises no cap — the test timeout still bounds the wait.
  await expect
    .poll(async () => {
      const r = await request.get(`${API}/api/v1/graph/compute_node/@local/fs-records/activity-status`);
      return ((await r.json()) as { data: unknown }).data;
    }, { message: 'index activity idle', timeout: 0 })
    .toBeNull();

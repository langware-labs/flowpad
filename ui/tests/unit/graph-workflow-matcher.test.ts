...
describe('graph-workflows v2 SDK surface', () => {
  it('registers the flow entities with the factory', () => {
    ...
    expect(GraphWorkflowNode.type).toBe('graph_workflow_node');
  ...
  it('exports the routing constants (mirror of graph_workflow_doc.py)', () => {
    ...
    expect(typeof graphWorkflows.inject).toBe('function');
    expect(typeof graphWorkflows.listRuns).toBe('function');
    expect(typeof graphWorkflows.fetchRunJournal).toBe('function');
  });
});

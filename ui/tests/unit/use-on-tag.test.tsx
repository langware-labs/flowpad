...
describe('useOnTag — subscription lifetime is component lifetime', () => {
  beforeEach(() => EventBus.clear());

  it('delivers while mounted, unsubscribes on unmount', () => {
    const handler = vi.fn();
    const { unmount } = renderHook(() => useOnTag('app.page.signal', handler));
  ...
  it('pattern change swaps the subscription (old await unhooks first)', () => {
    const handler = vi.fn();
    const { rerender } = renderHook(({ pattern }) => useOnTag(pattern, handler), {
  ...
  it('latest handler fires without resubscribing per render', () => {
    const first = vi.fn();
    const second = vi.fn();
    const { rerender } = renderHook(({ h }) => useOnTag('a.b', h), { initialProps: { h: first } });
  ...
  it('target filter change re-keys the subscription', () => {
    const handler = vi.fn();
    const { rerender } = renderHook(({ target }) => useOnTag('app.entity.created', handler, { target }), {

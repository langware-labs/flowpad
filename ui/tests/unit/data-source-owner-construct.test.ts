...
import { DataDriver, MessageThread } from '@sdk';

const AGENT = 'agent-22222222-2222-4222-8222-222222222222';

describe('owner survives construction', () => {
  it('DataDriver', () => {
    expect(new DataDriver({ name: 'x', provider: 'slack', owner: AGENT }).owner).toBe(AGENT);
    expect(new DataDriver({ name: 'x', provider: 'slack' }).owner).toBeNull();
  });
  it('MessageThread', () => {

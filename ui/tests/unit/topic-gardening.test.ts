// The topics gardening merge: blessed entities + bus-observed anonymous names
// → one taxonomy tree grouped by first segment. Pure-function slice of
// ui/src/components/browseable-tree/adapters/topicRoot.tsx.
import { describe, expect, it } from 'vitest';
import { mergeTopicRows } from '@src/components/browseable-tree/adapters/topicRoot';

const OBS = { count: 3, first_ts: 't0', last_ts: 't1', last_target: 'x:1' };

describe('mergeTopicRows', () => {
  it('groups by first segment, sorted, one row per name', () => {
    const byRoot = mergeTopicRows(
      [
        { id: '1', name: 'flow.done' },
  ...
  it('attaches observed stats to blessed rows and keeps anonymous rows separate', () => {
    const byRoot = mergeTopicRows([{ id: '1', name: 'flow.done' }], {
      'flow.done': OBS,
      'flow.mystery': OBS,
    });
  ...
  it('namespace topics group under their --ns-- marker', () => {
    const byRoot = mergeTopicRows([{ id: '1', name: '--acme--.orders.created' }], {});
    expect(byRoot.get('--acme--')!.map((r) => r.name)).toEqual(['--acme--.orders.created']);
  });

  it('tolerates empty inputs', () => {
    expect(mergeTopicRows([], {}).size).toBe(0);
  });
});

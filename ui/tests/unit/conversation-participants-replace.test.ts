/**
 * Conversation roster updates must REPLACE the cached ``participants``
 * array, never index-merge it.
 *
 ...
 * [B,B]) left the departed participant at the tail and cross-merged unrelated
...
function seedCachedConversation(typeId: TypeId, participants: object[]) {
...
describe('Conversation participants: wire roster replaces cached roster', () => {
  ...
  it('a 2→1 shrink drops the departed participant (no stale tail)', () => {
    ...
    expect(ref.entity.participants).toHaveLength(1);
    expect(ref.entity.participants[0].user_id).toBe(USER_B.user_id);
  });
  ...
  it('entries are fully replaced — no key bleed from the old entry at the same index', () => {
    ...
    fireDataOpUpdate(typeId, {
      type: ENTITY_TYPE,
      id: ENTITY_ID,
      participants: [{ user_id: USER_B.user_id, name: USER_B.name }],
    });

    expect(ref.entity.participants).toHaveLength(1);
    expect(ref.entity.participants[0].role).toBeUndefined();
  });
  ...
  it('a grow (re-invite) lands both entries', () => {
    ...
    expect(ref.entity.participants).toHaveLength(2);
    expect(ref.entity.participants.map((p: any) => p.user_id)).toEqual([USER_A.user_id, USER_B.user_id]);
  });

  it('an update without participants leaves the cached roster untouched', () => {
    const ref = seedCachedConversation(typeId, [USER_A, USER_B]);
  ...
  it('other scalar fields in the same frame still merge normally', () => {
    ...
    expect(ref.entity.participants).toHaveLength(1);
    expect(ref.entity.participants[0].user_id).toBe(USER_A.user_id);
  });
});

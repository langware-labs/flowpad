...
import { SecretPack, Project } from '@sdk';
...
describe('the Credentials quick-create entry', () => {
  ...
  it('opens the one credential dialog, in user or project scope', () => {
    const d = getDescriptor(SecretPack.type);
  ...
  it('a name-only create declares one env-file variable for the user', async () => {
    await getDescriptor(SecretPack.type)!.create({ project: null, name: ' Stripe key ', scope: 'user' });
  ...
  it('a project-scope create declares it on that project', async () => {
    ...
    const result = await getDescriptor(SecretPack.type)!.create({ project, name: 'Twilio', scope: 'project' });

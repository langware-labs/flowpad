...
describe('setup-github journey graph', () => {
  it('parses into the three guided steps with capability-setup acts', () => {
    ...
    expect(steps[1].act?.capability).toBe('source_control.github.gh');
    expect(steps[2].act?.capability).toBe('source_control.github.gh');
  ...
  it('declares the capability gate', () => {
    ...
    expect(raw.gate?.requires_capabilities).toEqual(['source_control.github', 'source_control.github.gh']);
  ...
  it('s2 confirm means INSTALLED: a discovered value, regardless of auth state', () => {
    ...
    const row = (value: unknown) => ({ kind: 'source_control.github.gh', value });
  ...
  it('s1/s3 confirms match only the available state of their kind', () => {
    ...
    expect(s1.validate({ kind: 'source_control.github', state: 'available' })).toBe(true);
    expect(s1.validate({ kind: 'source_control.github', state: 'none' })).toBe(false);
    const s3 = new QueryFilter({ match: steps[2].await!.confirm!.match! });
    expect(s3.validate({ kind: 'source_control.github.gh', state: 'available' })).toBe(true);
    expect(s3.validate({ kind: 'source_control.github.gh', state: 'not_available' })).toBe(false);
  });
});

 ...
 *
 *   1. alice creates a conversation with bob (existing invitation flow).
...
async function acceptPendingInvitations(bobPage: Page) {
  await gotoHome(bobPage);
  await bobPage.getByTestId('refresh-conversations-button').click();
  await bobPage
    .locator('[data-testid="conversation-row"][data-kind="invitation"]')
    .first()
    .waitFor({ state: 'visible', timeout: 2_000 });
  const accepts = await bobPage
    .locator('[data-testid="conversation-row"][data-kind="invitation"] [data-testid="accept-invitation-button"]')
    .all();
  expect(accepts.length).toBeGreaterThan(0);
  await Promise.all(accepts.map((b) => b.click()));
}
...
test('live session: alice prompts → bob approves → reply only in the session view', async () => {
  ...
  try {
    ...
    await acceptPendingInvitations(bob.page);

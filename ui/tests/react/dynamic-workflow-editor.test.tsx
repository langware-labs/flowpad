...
function stubs(script: string) {
  ...
  const workflow = {
    id: 'dw1', type: 'dynamic_workflow', name: 'demo', description: 'a demo', run, markEdit,
    typeId: { type: 'dynamic_workflow', id: 'dw1', toString: () => 'dynamic_workflow-dw1' },
  } as never;
  return { fsRef, workflow, write, run, markEdit };
}

describe('DynamicWorkflowAssetEditor', () => {
  it('loads the script, saves edits, and runs', async () => {
    const { fsRef, workflow, write, run, markEdit } = stubs('export const meta = { name: "demo" }');
    render(<DynamicWorkflowAssetEditor fsRef={fsRef} workflow={workflow} />);

    const ta = await screen.findByTestId('dw-script');

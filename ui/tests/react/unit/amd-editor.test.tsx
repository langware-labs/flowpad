...
import { ProcessState, ProcessorStatus } from '@sdk';
...
describe('AMDEditor', () => {
  ...
  describe('ProcessState prop validation', () => {
    /**
     * Create a mock ProcessState for testing
     */
    function createMockProcessState(overrides: Partial<ProcessState> = {}): ProcessState {
      return {
    ...
    const renderEditorWithProcessState = (
      initialContent: string,
      processState: ProcessState | null,
      completedInstructions: Set<string> = new Set(),
    ) => {
      const onSave = vi.fn();
      const result = render(
    ...
    it('accepts processState prop with IDLE status', async () => {
      const processState = createMockProcessState({ status: ProcessorStatus.IDLE });
      renderEditorWithProcessState(DEMO_AMD_CONTENT, processState);
    ...
    it('accepts processState prop with RUNNING status', async () => {
    ...
    it('accepts processState prop with COMPLETE status', async () => {
      const processState = createMockProcessState({
    ...
    it('accepts processState prop with ERROR status', async () => {
      const processState = createMockProcessState({
    ...
    it('accepts processState prop with stack frames', async () => {
      const processState = createMockProcessState({
        status: ProcessorStatus.RUNNING,
        index: 0,
        totalInstructions: 3,
    ...
    it('accepts null processState prop', async () => {
      renderEditorWithProcessState(DEMO_AMD_CONTENT, null);
    ...
    it('accepts completedInstructions set with processState', async () => {
      const processState = createMockProcessState({
        status: ProcessorStatus.RUNNING,
        index: 2,
        totalInstructions: 5,
      });
      const completedInstructions = new Set(['instr-1', 'instr-2']);

      renderEditorWithProcessState(DEMO_AMD_CONTENT, processState, completedInstructions);
    ...
    it('accepts processState with variables', async () => {
      const processState = createMockProcessState({
        status: ProcessorStatus.RUNNING,
        variables: {
          userName: 'testUser',
    ...
    it('accepts processState with debug state enabled', async () => {
      const processState = createMockProcessState({
        status: ProcessorStatus.STEPPING,
        debug: {
          enabled: true,
    ...
    it('renders editor with processState in PAUSED status', async () => {
      const processState = createMockProcessState({
        status: ProcessorStatus.PAUSED,
        waitingForInput: true,
        inputId: 'user-input-1',
      });
      renderEditorWithProcessState(DEMO_AMD_CONTENT, processState);

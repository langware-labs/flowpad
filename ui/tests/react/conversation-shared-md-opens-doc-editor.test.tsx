 ...
 *
 * which routes a `.md` body to `DockPointer.forAssetEditor('markdown', …)` →
 * `/dock/assets/editor/markdown/…` (the markdown document editor — rich Milkdown
 * rendering with working internal-link navigation), and every other file to
 * `DockPointer.forFile(…)` → `/dock/editor/…` (the code editor).
 *
 ...
 * resulting URL is read off `window.location` (NavigationActions commits the
 * dock URL via `history.pushState`). Only ambient boundary hooks the row needs
 * to mount are stubbed — never the routing decision.
 */

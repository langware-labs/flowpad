...
export function classifyWebappSeverity(signals: WebappSignals): WebappVerdict {
  ...
  if (probe) {
    ...
    if (!probe.reachable) {
      // A probe can land between a restart's teardown and its listen. Require
      // the liveness poll to agree before condemning an app that was working;
      // the failure count is what decides once liveness IS reporting down.
      if (everLoaded && health !== 'down') {
        return { severity: 'unknown', code: 'starting', detail };
      }
      if (everLoaded && consecutiveFailures < FAILURE_THRESHOLD) {
        return { severity: 'unknown', code: 'starting', detail };
      }

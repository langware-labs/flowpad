/**
 * Agentic Flows v2 store (zustand) — one open flow at a time.
 *
...
export interface StudioState {
  ...
  persistDoc?: (doc: FlowDoc) => void;
  persistDisplay?: (display: DisplayDoc) => void;
  openProcess?: (processId: string) => void;

  reset: (flowId: string | null) => void;
  setFlow: (opts: { flowId: string; name: string; enabled: boolean; doc: FlowDoc; display: DisplayDoc }) => void;
  setWriters: (w: {
    persistDoc: (doc: FlowDoc) => void;
    persistDisplay: (display: DisplayDoc) => void;
    openProcess: (processId: string) => void;
  }) => void;
  mutateDoc: (fn: (doc: FlowDoc) => FlowDoc) => void;
  moveNode: (nodeId: string, x: number, y: number) => void;
  setRuns: (runs: RunSummary[]) => void;
  applyRunEvent: (msg: FlowRunEventMessage) => void;
...
export type { FlowDoc, FlowDocEdge, FlowDocNode };

...
// Synthetic placeholder addresses generated by older builds for users with no
// real email (``hostname@desktop.local``). They are not deliverable — hide
// them from the typeahead and refuse them as free-form input so they can
// never be selected as a share recipient.
const isSyntheticEmail = (email: string | null | undefined): boolean =>
  !!email && email.toLowerCase().endsWith('@desktop.local');
...
export function ContactPicker({
  ...
  const contacts = useMemo(
    () =>
      allUsers.filter(
        (u) => (!excludeUserId || u.id !== excludeUserId) && !isSyntheticEmail(u.email),
      ),
  ...
  const addFreeFormEmail = () => {
    ...
    if (!v || !EMAIL_RE.test(v) || isSyntheticEmail(v) || alreadyAdded(v) || isFull) return;
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {!isFull &&
        ...
        !isSyntheticEmail(filterText.trim()) &&

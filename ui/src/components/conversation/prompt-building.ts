...
export async function buildMergedPrompt(flowMessage: FlowMessage): Promise<string> {
  ...
  const recordsDataDir = dataManager.recordsDataDir;
  const contextLines: string[] = [];
  for (const tid of flowMessage.contextEntities) {
    if (!tid?.type || !tid?.id || tid.type === FlowMessage.type) continue;
    if (recordsDataDir) {
      const recordPath = `${recordsDataDir}/${tid.type}/${tid.type}-@${tid.id}/data.json`;
  ...
  if (contextLines.length > 0) {
    const verb = recordsDataDir ? 'read each one as JSON' : 'fetch each one with curl/HTTP';
    contextSection.push(

...
export function SessionCard({ sessionId, session, role, promptCount, replyCount, onOpen, onApprove, onDecline }: SessionCardProps) {
  ...
  return (
    <div
      ...
      data-status={state}
      className={cn(
        'ms-10 inline-flex max-w-full items-center gap-2 rounded-md border bg-background px-2.5 py-1 text-[11px]',
        TONE[state],
      )}
    >
      ...
      <button
        ...
        className="ms-auto inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
      >

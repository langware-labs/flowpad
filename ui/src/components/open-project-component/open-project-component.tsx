...
function CompactProjectSelectDialog({
  ...
  const renderRow = (project: ProjectListItem, tabCount?: number) => {
    const projectPath = normalizePath(project.cwd || project.name || '');
    const isCurrent =
      !!currentProjectPath && canonicalPath(projectPath) === canonicalPath(currentProjectPath);
    const isOpening = openingProjectId === project.id;
    return (
      <button
        key={project.id}
...
export function OpenProjectComponent({
  ...
  const { computeNode, ensureProjectAndSetContext, pickFolder, openProjectFolder } = useProjectOpener({
    onProjectChanged,
    onPicked,
    onError: setError,
  });
  ...
  const handleOpenFolder = useCallback(async () => {
    // openProjectFolder never throws — cancel/failure return { opened: false }
    // and route errors through onError (setError). We only toggle busy state.
    setIsSubmitting(true);
    setError(null);
    try {
      const { opened } = await openProjectFolder(defaultWorkspacePath || undefined);
      if (opened) onOpenChange(false);
    } finally {
      setIsSubmitting(false);
    }
  }, [openProjectFolder, defaultWorkspacePath, onOpenChange]);

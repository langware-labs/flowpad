...
import { getTranscriptStats, type ParsedTranscript } from '@sdk';
  ...
  return (
    <div className="border-b border-border bg-card p-3">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
        ...
        {transcript.version && <span>v{transcript.version}</span>}
        {transcript.gitBranch && (
          <span className="rounded bg-blue-500/10 px-1.5 py-0.5 text-blue-600">{transcript.gitBranch}</span>
        )}
        {transcript.cwd && <span className="truncate font-mono">{transcript.cwd}</span>}
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {toolEntries.map(([toolName, enabled]) => (
              <label key={toolName} className="flex items-center gap-1 rounded bg-muted/50 px-2 py-1 text-xs">
                ...
                <span className="text-[10px] text-muted-foreground">{formatNumber(toolCounts[toolName] || 0)}</span>
              </label>
            ))}
          </div>
        </div>
      ...
      {stats.models.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1">
          {stats.models.map((model) => (
            <span key={model} className="rounded bg-muted px-1.5 py-0.5 font-mono text-[10px]">
              {model}
            </span>
          ))}
        </div>

  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex items-center gap-3 border-b px-4 py-2">
        <span className="text-sm font-semibold">Claude Code — Context Window</span>
        {data && (
        ...
        {!data && !isLoading && (
          <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
            <div className="text-center">
              <p className="font-medium">No context data</p>
              <p className="mt-1 text-xs">Ensure the claude CLI is installed and authenticated.</p>
            </div>
          </div>
        )}
        ...
        {!data && isLoading && (
          <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
            <div className="text-center">
              ...
              <p>Running /context…</p>
              <p className="mt-1 text-xs text-muted-foreground/60">Takes ~4 seconds</p>
            </div>
          </div>
        )}
...
function Dashboard({
  ...
  const skillsBySource = data.skills.reduce<Record<string, ClaudeContextSkill[]>>((acc, s) => {
    const src = s.source === 'undefined' ? 'Built-in' : s.source;
    (acc[src] ??= []).push(s);
    return acc;
  }, {});

...
function subAgentLocationCell(r: SearchRow): React.ReactNode {
...
registerColumns('agent', [
  { key: 'model', header: 'Model', render: (r: SearchRow) => (r as { model?: string }).model || '—' },
  {
    key: 'worker_type',
    header: 'Worker',
    render: (r: SearchRow) => (r as { worker_type?: string }).worker_type || '—',
  },
]);

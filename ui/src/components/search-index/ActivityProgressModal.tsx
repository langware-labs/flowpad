...
export function ActivityProgressBar({ table, onClick }: { table: IndexProgressTable; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className="w-full rounded-md px-1 py-0.5 text-start transition-colors hover:bg-accent/30">
      <div className="mb-1 flex items-center justify-between">
        <span className="text-xs font-medium tabular-nums">
          <Trans>{label} records</Trans>
        </span>
        {table.current ? (
          <span className="ms-2 max-w-[180px] truncate font-mono text-xs text-muted-foreground">{table.current}</span>
        ) : isDone ? (
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        <div className="flex max-h-[70vh] flex-col gap-1 overflow-y-auto pe-1">
          {table.rows.length === 0 && (
            <p className="px-3 py-2 text-xs italic text-muted-foreground">
              <Trans>No records discovered yet.</Trans>
            </p>
          )}
          {table.rows.map((row) => {
            ...
            const rowClass =
              state === 'current' ? 'border border-primary/30 bg-primary/5' : state === 'pending' ? 'opacity-60' : '';
            const countLabel =
              row.total > 0 ? `${row.done.toLocaleString()}/${row.total.toLocaleString()}` : row.done.toLocaleString();

            return (
              <div key={row.type_name} className={`flex items-center gap-2 rounded-md px-3 py-1.5 ${rowClass}`}>
                <Icon className={`h-3.5 w-3.5 shrink-0 ${iconClass}`} />
                <span className="flex-1 truncate font-mono text-sm">{row.type_name}</span>

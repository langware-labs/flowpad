/**
 * BulkReminderButton - Same popover UI as ReminderButton but without task binding.
 * Calls onSetReminder with just the date for bulk operations.
 */
...
export function BulkReminderButton({ onSetReminder }: BulkReminderButtonProps) {
  ...
  const handleQuickPick = (amount: number, u: 'days' | 'weeks') => {
    onSetReminder(computeDate(amount, u));
    setOpen(false);
  };

  const handleSliderCommit = (value: number[]) => {
    setCount(value[0]);
    onSetReminder(computeDate(value[0], unit));
    setOpen(false);
  };

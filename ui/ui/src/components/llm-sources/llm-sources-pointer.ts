 ...
 * `mapping` and `defaults` are whole-screen destinations. The selection lives in the URL rather
 * than component state (reload lands where you were, drill-down is a navigation), and
 * `foldsPointer` on the registry entry keeps every combination in one tab chip — the
 * credentials view's pattern.
 *
 * No React here: the version popover, the warnings popover and the notification hooks all
 * import this file, so it must stay a leaf (same rule as `llm-endpoints-pointer.ts`).
 */

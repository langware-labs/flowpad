"""POST /api/v1/topics/context — the `flow topic get` join, three modes.

Fixture: a markdown doc bound via frontmatter `topics:` (indexed through the
normal single-file index path) + a `.py` file carrying a `topic` capsule under
a scan root. Covers descendant inclusion, mode shaping, anonymous degradation,
and mentions for blessed topics.
"""
...
@pytest.fixture()
async def topic_root(tmp_path, client):
    docs = tmp_path / "docs"
    docs.mkdir()
    ...
    AssetCapsule.from_path(code_path).write(
        "topic",
        CapsuleData(1, {"topics": {"qa.ctx.runs": "Run loop entry point"}}),
    )
    return tmp_path, doc_path, code_path


async def test_context_modes_and_descendants(client, topic_root, tmp_path):
    root, doc_path, code_path = topic_root
    ...
    rec = records[0]
    assert rec.topics == ["qa.ctx.runs.budgets"]
    from flow_sdk.core.entity.entity_model import Entity  # noqa: PLC0415

    entity = await Entity.from_record(rec)
    assert entity is not None

    # line mode — asking the PARENT topic includes the descendant-tagged doc.
    resp = await client.post(
        "/api/v1/topics/context",
        json={"name": "qa.ctx.runs", "mode": "line", "root": str(root)},
    )
    assert resp.status_code == 200, resp.text
    data = resp.json()["data"]
    assert data["topic"]["name"] == "qa.ctx.runs"
    assert data["topic"]["blessed"] is False  # anonymous degrades gracefully
    ...
    assert site["topics"] == {"qa.ctx.runs": "Run loop entry point"}
...
async def test_blessed_header_and_invalid_inputs(client):
    from flow_sdk.builtin.topic import Topic  # noqa: PLC0415

    await Topic(name="--qa--.ctx.header", title="QA header", description="desc").save()
    resp = await client.post(
        "/api/v1/topics/context", json={"name": "--qa--.ctx.header", "mode": "line"})
    header = resp.json()["data"]["topic"]
    assert header["blessed"] is True and header["title"] == "QA header"

    bad = await client.post("/api/v1/topics/context", json={"name": "not a topic!"})
    assert bad.json()["status"] == "FAIL"
    bad_mode = await client.post(
        "/api/v1/topics/context", json={"name": "qa.ctx", "mode": "huge"})

"""Observed-topics debug surface: emit → observe roundtrip over the API.
...
async def test_emit_then_observed_roundtrip(client):
    emitted = await client.post(
        "/api/v1/debug/emit_topic",
        json={"topic": "qa.observed.check", "target": "test:1"},
    )
    assert emitted.status_code == 200, emitted.text

    observed = await client.get("/api/v1/debug/observed_topics")
...
async def test_emit_topic_validates_grammar_with_force_escape(client):
    bad = await client.post(
        "/api/v1/debug/emit_topic",
        json={"topic": "Not A Topic!", "target": "test:1"},
    )
    assert bad.json()["status"] == "FAIL"

    forced = await client.post(
        "/api/v1/debug/emit_topic",
        json={"topic": "Not A Topic!", "target": "test:1", "force": True},
    )

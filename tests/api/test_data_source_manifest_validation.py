...
the dialog: ``DataDriver.save`` raises ``ValueError`` naming the field and the
...
async def test_a_missing_required_field_is_a_400_naming_the_field(client):
    ...
    resp = await client.post("/api/v1/graph/data_driver", json={"name": "s", "provider": "api_strict_provider", "config": {}})
    assert resp.status_code == 400, resp.text
    assert "config.root is required" in resp.text

    resp = await client.post("/api/v1/graph/data_driver", json={"name": "s", "provider": "api_strict_provider", "config": {"root": "/x", "feed": "ftp://y"}})
    assert resp.status_code == 400, resp.text
    assert "config.feed is not valid: ftp://y" in resp.text

    resp = await client.post("/api/v1/graph/data_driver", json={"name": "s", "provider": "api_strict_provider", "config": {"root": "/x", "feed": "http://y"}})

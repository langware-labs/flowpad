...
async def test_markdown_index_cold_build(
    ...
    # Never launched, so readiness is the transport's: a fresh PTY process waits for
    # its worker, a fresh headless one (deepagents is headless-only) takes the first

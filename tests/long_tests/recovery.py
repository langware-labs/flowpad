...
for assertions — Shell.is_alive, Shell.read(), Shell.pty.kill(),
AgenticProcess.get_shell(), AgenticProcess.sync_status().

Scenario A: PTY lifecycle
  1. Create AgenticProcess via HTTP
  2. open() via HTTP → shell_id + session_id
  3. Load Shell entity, hydrate provider
  4. assert shell.is_alive
  5. Poll read() until Claude startup output appears
  6. proc.send('echo hello_claude') via object call
  7. Poll read() for echo token
...
async def _poll_output(shell, keyword: bytes, timeout: float = 30.0) -> bytes:
    """Poll shell.read() until keyword appears or timeout."""
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
...
async def test_proc_pty_lifecycle(recovery_server):
    ...
    async with httpx.AsyncClient(base_url=base_url, timeout=30.0) as http:
        ...
        assert session_id, "open() must return a session_id"

    # Load Shell entity directly and check liveness
    shell = await _load_shell(shell_id, db_path)
    assert shell.is_alive, "Shell must be connected immediately after open()"
...
async def test_proc_recovery_after_destruct(recovery_server):
    ...
    shell = await _load_shell(shell_id, db_path)
    assert shell.is_alive, "Shell must be connected after open()"
    ...
    await pty.kill()
    assert not shell.is_alive, "Shell must not be connected after pty.kill()"
    ...
    assert open2_data["session_id"] == session_id, (
        "session_id must be preserved through recovery"
    )

    # Phase 5: New shell is connected and interactive
    shell2 = await _load_shell(shell_id_2, db_path)
    assert shell2.is_alive, "Recovered shell must be connected"
...
async def test_proc_recovery_after_server_restart(recovery_server):
    ...
    assert not shell_before.is_alive, "Shell must not be connected after server SIGKILL"
    ...
    try:
        ...
        async with httpx.AsyncClient(base_url=base_url, timeout=30.0) as http:
            ...
            assert proc_data["session_id"] == session_id
        ...
        assert open2_data["session_id"] == session_id, (
            "session_id must be preserved through restart"
        )

        # New shell is connected and interactive
        shell_after = await _load_shell(shell_id_after, db_path)
        assert shell_after.is_alive, "Recovered shell must be connected after restart"
...
async def test_agentic_process_recovery_no_http(tmp_path):
    ...
    try:
        ...
        await proc.send("Remember the number 3422455")
        ...
        assert session_rec is not None, (
            f"ClaudeSessionRecord not found after 30s for session {session_id}. "
        ...
        await pty.kill()
        assert not shell.is_alive, "Shell must be disconnected after pty.kill()"
        ...
        await proc.send("What is the number I told you to remember? Reply with just the number.")

        # Phase 4: Claude resumes with full context and recalls the number
        shell2 = await proc.shell()

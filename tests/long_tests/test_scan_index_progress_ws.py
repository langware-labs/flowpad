...
@pytest.fixture(autouse=True)
def wipe_test_db():
    """Full DB reset before each test: dispose engine, delete DB file, clear all cached state.

    Calls _reset_db_state() from api/conftest.py to also clear _driver_instances and
    reset DBEntity._db — necessary because SQLiteDBDriver replaces the LazyDBDriver
    descriptor on first access and must be reset so test 2+ get a fresh driver pointing
    at the new DB file.
    """
    import asyncio
    import os
    import flow_sdk.db.database as db_mod
    from tests.api.conftest import _reset_db_state

    # Dispose any open engine first (so aiosqlite threads release the file handle)
    ...
    # Reset all DB state: _engine, _session_factory, _driver_instances, DBEntity._db
    _reset_db_state()

    # Delete the DB file so each test starts with an empty schema
    ...
    # Clear bootstrap cache so each test starts fresh (cache TTL is 30s — too long for tests)
    try:
        import flow_sdk.server.routes.bootstrap as bootstrap_mod
        bootstrap_mod._bootstrap_cache = None
        bootstrap_mod._bootstrap_cache_ts = 0.0
    except Exception:
        pass

...
def test_it_runs_once_on_first_boot_and_each_question_waits_for_an_answer():
    """`fire_once`, like `dev-toolchain`: first boot only, not every boot. What
    makes that acceptable for a question with a 60s ask default is the OTHER
    flag — `until_answered` on every ask op here — so the one attempt this
    trigger gets is never lost to the clock, only to nobody being there to
    answer at all (see `test_nobody_to_ask_gives_up_at_once...`)."""
    trigger = json.loads((ASSETS / "wizard/llm-setup/agentic-assets/trigger/on-app-ready/trigger.json").read_text())
    assert trigger["tag"] == {"on": "app.ready"}

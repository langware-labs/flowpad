"""``DataDriverSpec.sends`` is the driver's ``sends`` flag, put on the wire so a
...
def test_sends_mirrors_the_driver(provider):
    ...
    assert DataDriverSpec(name=provider).sends is driver.sends


def test_a_provider_nothing_registered_does_not_send():
    assert DataDriverSpec(name="no-such-provider").sends is False


def test_sends_reaches_the_wire_but_not_the_manifest(tmp_path):
    spec = DataDriverSpec(name="slack", title="Slack")

"""An agent op reports what the AGENT returned, not that it stopped.
...
and these are its rules — an agent op's, because an agent run IS a ComputeOp
(subkind ``agent``).
...
class Port(DataSpec):
    spec_kind: ClassVar[str] = "test.agent_op.port"
    port: int
...
def _op(*, output: object = "string", check: bool = False) -> ComputeOpSpec:
    body: dict = {
        "name": OP_NAME,
        "subkind": "agent",
        "exe_data": {"agent": "capability-installer", "prompt": "do the thing"},
    }
    if output:
        body["output_spec_kind"] = output
    if check:
...
async def _ok_shell(command, **_kw):
...
def test_the_agents_own_summary_is_its_reply(tmp_path):
    ...
    assert answer.executor == "agentic_process-proc-1"
...
def test_a_stale_receipt_cannot_pass_a_rung_that_did_nothing(tmp_path):
    ...
    answer = _run_op(tmp_path, _op(), _writer(None))
    assert not answer.ok
    assert "from LAST time" not in answer.detail and "from LAST time" not in answer.text
...
def test_a_declared_shape_is_enforced_on_the_way_out(tmp_path):
    """A value that does not match the declared kind is a FAILURE, not a
    warning: a caller binding it into a later step would carry the breakage
    somewhere it cannot be explained.
    """
    typed = _op(output="test.agent_op.port")

    good = _run_op(tmp_path, typed, _writer({"status": "done", "data": {VALUE_KEY: {"port": 8099}}}))
    assert good.ok and isinstance(good.value, Port) and good.value.port == 8099

    bad = _run_op(tmp_path, typed, _writer({"status": "done", "data": {VALUE_KEY: {"port": "nope"}}}))
    assert not bad.ok
    assert "test.agent_op.port" in bad.detail
...
def test_the_rung_ticks_while_the_agent_works(tmp_path):
    ...
    async def launch(*, on_status=None, **_kw) -> PromptResult:

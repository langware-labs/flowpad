...
async def test_skips_hub_get_when_not_ready(status) -> None:
    ...
    with patch(
        ...
        result = await _download_and_unpack_bundle(
            FM_ID, "conversation-deadbeef.flowmsg", body_status=status,
        )
...
async def test_proceeds_when_ready() -> None:
    ...
    with patch(
        ...
        result = await _download_and_unpack_bundle(
            FM_ID, "body.flowmsg", body_status=BodyStatus.READY,
        )
...
async def test_proceeds_when_status_omitted() -> None:
    ...
    with patch(
        ...
        await _download_and_unpack_bundle(FM_ID, "body.flowmsg")
    assert mock_get.await_count == 1

...
async def test_backfill_counts_then_stamps_then_finds_nothing():
    ...
    legacy_agent_source = DataDriver(name="a", provider="cloud_email", config={"agent_id": agent_id})
    legacy_user_source = DataDriver(name="u", provider="rss")
    ...
    assert (await DataDriver.get_one({"id": legacy_agent_source.id})).owner == agent_tid

...
async def test_pages_are_size_n_and_a_page_ack_commits_its_last_row():
    with scripted_provider("pager") as script:
        ...
        inbox = StreamInbox(f"{mint_uuid()}@pager", provider="pager")
        name = _name()
        async with workflow(name):
            pages = await _take(inbox.pages(size=50, poll_every=0), 3)
...
async def test_an_unacked_page_is_redelivered_on_the_next_run():
    with scripted_provider("pager-redeliver") as script:
        ...
        inbox = StreamInbox(f"{mint_uuid()}@pager", provider="pager-redeliver")
        name = _name()
        async with workflow(name):
            first, second = await _take(inbox.pages(size=50, poll_every=0), 2)
            await first.ack()
        async with workflow(name):
            (again,) = await _take(inbox.pages(size=50, poll_every=0), 1)
...
async def test_filtered_rows_are_dropped_from_the_page_but_covered_by_its_ack():
    with scripted_provider("pager-filter") as script:
        ...
        inbox = StreamInbox(f"{mint_uuid()}@pager", provider="pager-filter", senders=["alice@example.com"])
        name = _name()
        async with workflow(name):
            (page,) = await _take(inbox.pages(size=50, poll_every=0), 1)

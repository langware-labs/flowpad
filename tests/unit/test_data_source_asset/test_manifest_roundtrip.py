"""A ``DataDriver`` is a folder asset whose main doc is the FLAT manifest:
...
from flow_sdk.schema.data_spec.data_driver_spec import AuthSpec, DataDriverSpec, FieldHints
...
def test_store_writes_a_flat_manifest_and_load_reads_it_back(tmp_path):
    ...
    back = ser.load(DataDriver, local_origin_for_path(root))
    for name in DataDriverSpec.model_fields:

...
def test_a_script_source_may_declare_traits():
    m = parse_manifest(
        {**RSS, "name": "wiki", "traits": {"emits": "content.page", "id_unique_within": "source"}},
        files={"fetch.py"},
    )
    assert m.runtime is Runtime.SCRIPT
    assert m.traits.id_unique_within is IdUniqueness.SOURCE
    assert m.traits.owns_bytes is True

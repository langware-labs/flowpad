...
def test_search_title_reads_jsonl(tmp_path):
    """search_title returns the custom-title entry from JSONL."""
    from flow_sdk.fs_records.claude.claude_session import ClaudeSessionRecord

    sid = str(uuid.uuid4())
    p = tmp_path / f"{sid}.jsonl"
    _write_session_jsonl(p, sid, ["explain the v0.2.3 release", "sure, here is the summary"])
    # Append a custom-title entry (auto-generated by Claude or user /rename)
    with open(p, "a", encoding="utf-8") as fh:
        fh.write(json.dumps({"type": "custom-title", "customTitle": "Explain v0.2.3 release"}) + "\n")

    rec = ClaudeSessionRecord.from_jsonl(p)
    assert rec.search_title is not None
    assert "v0.2.3" in rec.search_title

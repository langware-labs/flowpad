...
def test_q_bundle_is_a_valid_agent_with_its_qa_skill() -> None:
    main_ref = FSRef(Q_ROOT / "agent.json", record_type="agent", read_only=True)
    fields = parse_agent_document(
        main_ref._path.read_text(encoding="utf-8"),
        "q",
        (Q_ROOT / "system_prompt.md").read_text(encoding="utf-8"),
    )

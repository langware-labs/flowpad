...
* ``_merge_shared_context_into_conversation`` appends + saves + links exactly the
  parsed items, and is a no-op (no save) when nothing is new.
"""
...
async def test_merge_appends_saves_and_links():
    ...
    with (
        ...
        await _merge_shared_context_into_conversation(
            conv,
            asset_references=[_DOC],
            shared_context_entities=[_SPEC],
...
async def test_merge_noop_when_nothing_to_add():
    ...
    with (
        ...
        # Only transport/self typeids → nothing to merge.
        await _merge_shared_context_into_conversation(
            conv,
            asset_references=[f"conversation-{_CONV_ID}", _FM],
...
async def test_merge_idempotent_second_call_skips_save():
    ...
    with (
        ...
        await _merge_shared_context_into_conversation(conv, [_DOC], [], "user-1")
        await _merge_shared_context_into_conversation(conv, [_DOC], [], "user-1")

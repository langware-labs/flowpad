...
from flow_sdk.fs_store.indexer.functions.skill import skill_id as _skill_id
...
def _rec(journal: Path):
    return extract_workflow_run(FSRef(journal, record_type=RecordType.WORKFLOW_RUN))[0]
...
def test_skill_bundled_run_links_to_workflow_and_skill(tmp_path):
    ...
    assert rec.skill_id == _skill_id(FSRef(skill_dir))  # frontmatter/name-derived

...
An activity per ``(subject_entity, path)`` is the same statement as one job of that name per entity,
...
async def test_a_claim_takes_the_address():
    act = Activity.try_claim("index", subject_entity="node-1")

    assert monitor.holder("index", subject_entity="node-1") is act
...
async def test_a_second_claim_is_refused_with_the_message_callers_match_on():
    ...
    Activity.try_claim("index", subject_entity="node-1")
...
async def test_scope_separates_addresses():
    ...
    Activity.try_claim("index", subject_entity="node-2")  # must not raise
...
async def test_a_finished_holder_frees_the_address():
    ...
    assert monitor.holder("index", subject_entity="node-1") is None
    Activity.try_claim("index", subject_entity="node-1")  # must not raise
...
async def test_a_holder_past_its_budget_is_claimable_again():
    ...
    stale = Activity.try_claim("index", subject_entity="node-1", timeout_seconds=0)
    time.sleep(0.01)
...
async def test_taking_over_a_stale_address_mints_a_fresh_node():
    ...
    fresh = Activity.try_claim("index", subject_entity="node-1")
...
async def test_claim_ends_and_releases_the_address_on_exit():
    async with Activity.claim("index", subject_entity="node-1") as act:
        act.total(10)
        act.inc_success(10)
...
async def test_claim_without_queue_raises_when_held():
    ...
    with pytest.raises(RuntimeError, match="already running"):
        async with Activity.claim("index", subject_entity="node-1"):
            pass
...
async def test_a_queued_caller_waits_for_the_holder_then_runs():
    held = Activity.try_claim("index", subject_entity="node-1")
    order: list[str] = []

    async def waiter():
        async with Activity.claim("index", subject_entity="node-1", queue=True):
...
async def test_a_queued_caller_gets_its_own_activity_not_the_holders():
    ...
    else's subject_entity, so serving its result would be answering a question nobody asked."""
    held = Activity.try_claim("index", subject_entity="node-1")
    held.inc_success(50)
    seen: list[int] = []

    async def waiter():
        async with Activity.claim("index", subject_entity="node-1", queue=True) as act:
...
async def test_waiting_is_bounded_by_the_holders_own_budget():
    ...
    Activity.try_claim("index", subject_entity="node-1", timeout_seconds=0)

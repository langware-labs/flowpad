"""``StreamInbox.listen()`` on a durable position — the at-least-once proof.
...
from flow_sdk.blocks import Delivered, EmailMessageSpec, StreamInbox, workflow
...
async def _take(stream_inbox: StreamInbox, n: int) -> list[Delivered]:
...
async def _drained(stream_inbox: StreamInbox) -> bool:
...
async def test_an_unacked_item_is_redelivered_after_a_restart_and_an_acked_one_is_not():
    ...
    with scripted_provider("scripted") as driver:
        ...
        async with workflow(name):
            (first,) = await _take(stream_inbox, 1)          # handed out — then the process dies
            assert first.body == "hello" and first.redelivered is False

        async with workflow(name):                    # restart
            (again,) = await _take(stream_inbox, 1)
        ...
        async with workflow(name):                    # and once acked, gone for good
            assert await _drained(stream_inbox)
...
async def test_ack_is_an_offset_that_commits_everything_before_it():
    ...
    with scripted_provider("scripted") as driver:
        ...
        async with workflow(name):
            a, b, c = await _take(stream_inbox, 3)
            await c.ack()
...
async def test_outside_a_workflow_the_position_is_ephemeral():
    ...
    with scripted_provider("scripted") as driver:
        ...
        source = await stream_inbox.ensure_source()
...
async def test_our_own_sent_copy_is_filtered_and_acked():
    ...
    with scripted_provider("scripted") as driver:
        src = DataSource(
            name=f"pre {uuid.uuid4().hex[:8]}", provider="scripted", config={"address": addr}, account_key="me@scripted.test"
        )
        await src.save()
        stream_inbox = StreamInbox(addr, provider="scripted")
...
async def test_the_drain_is_paged(monkeypatch):
    ...
    with scripted_provider("scripted") as driver:
        ...
        agen = stream_inbox.listen(poll_every=0, page=100)
        got = []
        try:
            while len(got) < 250:

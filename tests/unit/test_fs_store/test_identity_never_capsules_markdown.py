...
``reconcile`` seam the index walk calls, over every registered
...
def _materialize(info, root: Path) -> Path:
    ...
    if isinstance(info.shape, Folder):
        ...
        if info.shape.main:
            (root / info.shape.main).write_text(
                "{}\n" if info.shape.ext == ".json" else "body\n", encoding="utf-8"
            )
        return root
    path = root.with_suffix(info.shape.ext)
...
def test_markdown_mint_writes_frontmatter_not_a_capsule_block(tmp_path: Path) -> None:
    ...
    minted = resolve_id(info, FSRef(doc))
...
def test_no_registered_type_stamps_a_capsule_block_into_markdown(tmp_path: Path) -> None:
    ...
    for name, info in _writable_types():
        ...
        try:
            ...
            resolve_id(info, FSRef(ref, record_type=name))
        except Exception:

...
def test_compute_hash_changes_when_named_file_changes(tmp_record):
    """compute_record_hash() reflects changes to named FSRef files."""
    rec = ConfigRecord(id="cfg-hash", type="config_rec", name="H")
    rec.save()
    rec.config = {"v": 1}
    rec.save()
    hash1 = rec.compute_record_hash()

    # Change the named file directly
    named = rec.record_data_dir / "config.json"
    named.write_text(
        json.dumps({"data": {"v": 999}}), encoding="utf-8"
    )
    hash2 = rec.compute_record_hash()

    assert hash1 != hash2, "hash must change when named FSRef file changes"


def test_fingerprint_changes_when_named_file_changes(tmp_record):
    """fingerprint property reflects changes to named FSRef files."""
    rec = ConfigRecord(id="cfg-fp", type="config_rec", name="FP")
    rec.save()
    rec.config = {"v": 1}
    rec.save()
    fp1 = rec.content_fingerprint

    import time
    time.sleep(0.01)  # ensure mtime differs

    # Overwrite the named file to change mtime+size
    named = rec.record_data_dir / "config.json"
    named.write_text(
        json.dumps({"data": {"v": 9999, "extra": "padding"}}), encoding="utf-8"
    )
    fp2 = rec.content_fingerprint

    assert fp1 != fp2, "fingerprint must change when named FSRef file changes"

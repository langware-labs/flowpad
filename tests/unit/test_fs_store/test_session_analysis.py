...
def test_load_from_sample():
    rec = SessionAnalysis.load_record(SAMPLE_DIR)
...
def test_analysis_json_empty_when_missing(tmp_path):
    ...
    rec = SessionAnalysis._init_record({"id": "a1", "type": "session_analysis", "name": "test"}, folder)
...
def test_clone_copies_record(tmp_path):
    src = tmp_path / "src"
    rec = SessionAnalysis._init_record({"id": "a1", "type": "session_analysis", "name": "test"}, src)
    dst = tmp_path / "dst"
    clone = rec.clone(dst)
    assert clone.id != "a1"

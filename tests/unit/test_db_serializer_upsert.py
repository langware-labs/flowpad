...
def test_natural_key_reads_the_same_on_spec_and_row():
    ser = DbSerializer()
    assert ser.natural_key_of(SourceItem, _spec()) == ("ds", "rss", "seg", "x1")
    assert ser.natural_key_of(SourceItem, SourceItem(**_spec().model_dump())) == ("ds", "rss", "seg", "x1")
...
def test_upsert_writes_the_key_it_resolves_by():
    ...
    assert (row.origin_kind, row.origin_namespace, row.origin_key) == ("rss", "seg", "x1")

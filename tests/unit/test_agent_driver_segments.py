...
async def test_rows_written_before_the_names_converged_keep_their_cursors(legacy):
    assert [s.key for s in await driver_type("agent").segments(_source(**legacy))] == ["ARCHIVE"]


@pytest.mark.asyncio
async def test_segments_wins_over_the_legacy_spelling():
    got = await driver_type("agent").segments(_source(segments=["INBOX"], streams=["ARCHIVE"]))
    assert [s.key for s in got] == ["INBOX"]
...
async def test_a_channel_connector_has_no_default_segment():
    ...
    got = await driver_type("agent").segments(_source(connector="slack", segments=["C0123ABCD"]))
    assert [s.key for s in got] == ["C0123ABCD"]
    with pytest.raises(SourceError):
        await driver_type("agent").segments(_source(connector="slack"))
...
async def test_a_missing_or_unknown_connector_is_a_config_error(config):
    ...
    with pytest.raises(SourceError):
        await driver_type("agent").segments(src)

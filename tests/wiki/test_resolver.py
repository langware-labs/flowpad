...
async def _insert_entity(*, type: str, id: str, uname: str) -> str:
    entity_id = id if is_valid_entity_id(id) else mint_uuid(f"wiki-resolver-test:{type}:{id}")
    async with session() as s:
        await s.execute(
            sa_insert(EntitySchema).values(id=entity_id, type=type, uname=uname)
        )
    return entity_id
...
class TestResolution:
    async def test_unique_name_resolves(self, store):
        target_id = await _insert_entity(type="agentic_process", id="proc-1", uname="my-process")
        ...
        assert resolved.target_id == target_id
    ...
    async def test_subpath_link_resolves_by_first_segment(self, store):
        target_id = await _insert_entity(type="skill", id="skill-1", uname="my-skill")
...
class TestAmbiguity:
    async def test_same_type_no_longer_wins(self, store):
        await _insert_entity(type="skill", id="s-1", uname="setup")
        await _insert_entity(type="markdown", id="d-1", uname="setup")
        await _insert_entity(type="agent", id="a-1", uname="setup")

        link = WikiLink(raw="setup", line=1)
        resolved = await resolve_link(link, src_type="markdown", src_id="src")
        assert resolved.target_type is None
        assert resolved.target_id is None

    async def test_no_longer_falls_back_to_alphabetical(self, store):
        await _insert_entity(type="skill", id="s-1", uname="setup")
        await _insert_entity(type="markdown", id="d-1", uname="setup")

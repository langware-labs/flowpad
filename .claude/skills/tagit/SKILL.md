...
**4 — Index it**, or it stays invisible to `flow tag get`:

```bash
flow record index docs/breadcrumbs/<slug>.md
```

**5 — Drop the breadcrumb.** Always through the script; never hand-write the
comment block:
...
Exit 2 (`NO_PROCESS`) outside a Flowpad worker, exit 5 if the backend is down —
on either, just print the path. Showing is not approval: ask the user to review

...
* **Add a new tag** — insert `toplog.log([...], …)` lines at the trace points
  that *would have* made the cause visible, AND add a `### <tag>` entry to
  `../tags.md`. Code and catalog land together so `scan` stays green.

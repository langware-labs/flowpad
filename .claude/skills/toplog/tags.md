...
- **Traces:** the spine of every tab switch, for EVERY tab kind — 4–7 frontend lines per switch (`toplog.client: [tab_switch] …`), each stamped `sw=<n> +<ms>ms` (ms since that switch started, on the page clock):
  - `start sw= via=openDock|view_mode|replace|detached|back|forward|popstate from=<kind:ptr> to=<kind:ptr> visibility=` — a navigation that will actually move the app. `back`/`forward`/`popstate` don't know the target yet (`to=-`); the `committed` line names it.
  ...
  - `painted sw= kind= body=content_panel|vibe_workspace|asset_vibe_workspace|vibe_new_chat|vibe_no_process path=` — first frame of the new body. Every kind has it; for sync views (project, assets, preferences, graph, hub…) it IS the end of the switch.
  ...
  - `error sw= sink=… err=` at every error sink: `dock_load_error` (with `action=` render_error/redirect/notify/banner/noop), `tab_open_failed` / `tab_close_failed` ("Tab failed to open"), `error_screen` (the route errorElement replaced the page), `router_on_error`, `terminal_runtime` (start/attach failed → banner), `process_runtime_soft` (loader kept the URL, banner), `process_missing` / `shell_missing` (→ redirected to a fallback), `no_realtime` (WS not connected in 5s), `pty_replay`, `conversation_messages`, `chat_history`/`vibe_history`, and `process_history` (SDK side, no `sw=` — history load failed and the pane shows as an empty session).
  - `uncaught sw= kind=error|rejection err=` — window `error` / `unhandledrejection`, attributed to the switch in flight.
- **Where:** state in `ui/src/navigation/tab-switch-state.ts` (`tabSwitch`, `beginTabSwitch`, `sinceTabSwitch` — state only, no logging). Lines: `ui/src/navigation/NavigationActions.ts` (`logTabSwitchStart`, openDock no-ops, goBack/goForward, commitDetached), `ui/src/main.tsx` (popstate, uncaught, router onError), `ui/src/router.tsx` (committed), `ui/src/routes/loaders/main-loader.ts` (loader outcome), `load-shell.ts`, `dock-load-error.ts`, `ui/src/tabs/tab-content-lifecycle.ts`, `ui/src/pages/flow-page/flow-page.tsx` (painted), `vibe-chat-pane.tsx`, `ui/src/components/terminal/TabbedTerminal.tsx`, `interactive-terminal/InteractiveTerminal.tsx`, `SimpleChatPane.tsx`, `ui/src/components/conversation/ConversationView.tsx`, `error-screen.tsx`, `ts_sdk/src/process/agentic-process.ts` (`process_history`).

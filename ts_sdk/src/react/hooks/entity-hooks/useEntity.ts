...
export function useEntity<T extends APIEntity<T>>(
      ...
      const initialize = async () => {
        try {
          ...
          if (cachedEntity) {
            ...
            // store's negative cache keeps us from re-fetching on every render.
  ...
  const refetch = useCallback(async () => {
    ...
    // A manual refetch is an explicit retry — drop any terminal 404 marker so
    // the store actually re-hits the network instead of short-circuiting.

...
inputs_hash: 6c5406a16ecdccb19bf2c22c22a1f9d96c97884af4e1cb426e97bd628fe5419a
template_version: 1
prompt_version: 1
...
- [Agent deployment — snippets](agent-deployment.md) — Snippets for placing an agent: Deployment rows, launching a session that answers a PromptResult, serve loops answering channels, reading and stopping placements, dedicated machines.
...
- [Call → ExitCode → Return](call-returns.md) — The call-return contract, every fence tested: ComputeOp as one call per subkind, ReturnedValue subclasses, busy as a field, no raised outcomes, role timeouts, ask, wizard, agent continuation.
...
- [Processes and agents — snippets](processes.md) — Runnable snippets for processes and agents: attaching MCP servers to a process or agent, per-harness rendering, and launching an agent to read its PromptResult answer.

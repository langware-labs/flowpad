...
| `Task.origin`, `MessageAttachment.origin`, `DataDriver.origin` | `OriginField` | PRIVATE re-declaration | a project/task origin is **cloned** by the receiver through its own action wire; an attachment's is a bundle pass-through; a data source's is re-derived by its driver's `origin_for` on every save (see [data sources](data-sources.md#the-two-destinations)) |

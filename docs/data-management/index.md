...
inputs_hash: 8c82b04ca132fd070680d195b7d0ceb8b15e55668050313cc103bcf9b9d79e3d
template_version: 1
prompt_version: 1
parent_ref: markdown_index-6136dbba-27ed-59c3-a192-fe2894f3ec30
vault_root: /Users/shlom/Documents/dev/flowpad-oss/docs
generated_at: '2026-09-19T23:43:14.698454+00:00'
latest_process_ref: ''
file_count: 26
subfolder_count: 0
---
...
- [DataSpec — shape as Pydantic, the spec as the layout](data-spec.md) — DataSpec as the one shape system: Pydantic subclasses, spec_kind registration in the SchemaRegistry, runtime-compiled shapes, and asset layout as a class.
- [Database Architecture](database.md) — SQLite layer wiring: WAL pragmas, BEGIN IMMEDIATE, pooled connections, one transaction per request, and the database-is-locked cascade that forced the async refactor.
...
- [Scan and Discovery](scan-and-discovery.md) — How FSIndexer's DFS finds records on disk: roots, waypoint fan-out, per-type parser and freshness slots, RecordQuery, and why indexing is explicit-action only.
- [Schema Registry](schema-registry.md) — SchemaRegistry, the single source of truth for per-type metadata: TypeInfo authoring, registration, and how DataSpec kinds bind into the same namespace.
- [The source contract and the sync runtime](source-contract-boundary.md) — The line between the source contract in flow_sdk/sources and the ingest runtime: the async session API, frozen value specs, capability protocols, the import ban.
- [Stream inbox projection — ingested messages become conversations](stream-inbox-projection.md) — One-way projection of ingested SourceItems into stream inbox conversations: reference-model FlowMessages, lookup-not-derive identity, the dedupe lock, and which kinds are admitted.

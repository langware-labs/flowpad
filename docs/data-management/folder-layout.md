...
`destination_in` (`flow_sdk/assets/creation.py`) owns the tail: the family folder itself for a singleton type, `<safe-name>/` for a `Folder` shape, else `<safe-name><ext>`. Types whose placement resolves to no subdir have no asset file. `TypeInfo.main_subdir` survives as a derived, read-only view of the same triple (claude default). Examples: `skill` → `.claude/skills` / `folder`; `markdown` → `docs` / `file`. Only the `asset_ref` path is persisted (as the `asset_ref` key in `metadata.json`); the shadow-folder path is computed at runtime.

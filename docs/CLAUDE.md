...
20. Entity–Record ID sync: `Entity.allocate_id(data)` keeps a conforming (v4/v5) provided id, normalizes a non-conforming one to `uuid5(type:id)`, and mints `uuid4` otherwise; entity subclasses with a stable filesystem identity (e.g. `Project` with `fs_storage_mount_path`) override it to a deterministic uuid5 that matches `FSRecord.fingerprint` for the same asset. This guarantees indexing an existing entity's record never creates a duplicate — `Entity.from_record()` finds the entity by its deterministic id.

...
| ------------------------------------------------------------------ | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pty_mode` (:453)                                                  | `bool = True`, persisted | **Transport intent** and the routing key. `True` → interactive PTY (live terminal). `False` → headless JSON-stream (`-p`/stream-json, no PTY, no xterm; loader skips the PTY attach). Execution routes on `pty_mode`, not `visible`; `pty_mode` seeds `visible` at launch and the chat⇄terminal toggle keeps them in lock-step in both directions (FLOWPAD-2105); the one case that still leaves them apart is a dock OPENED in chat on a session already `pty_mode=true` — first sight never mutates a worker. |
...
* **shell\_mode:** AgenticProcess owns the choice. On the default `shell_mode=False` (direct PTY spawn) path, the worker PID is stamped via `shell.set_worker_pid_direct(cmd)`.
...
| Codex   | `-c developer_instructions=<generated text>`                                                                    |
| Copilot | `COPILOT_CUSTOM_INSTRUCTIONS_DIRS=<assets>` so Copilot discovers `.github/instructions/flowpad.instructions.md` |

This keeps the user prompt unchanged; `compose_prompt()` remains a compatibility
hook and current drivers pass it through.
...
| `switch-mode`            | POST | `_enter_cli_mode` (→cli) / `start_pty` (→interactive)                           | both                  | body `{mode}`; **rejected mid-turn (409) in BOTH directions** — `_reject_if_turn_in_flight`, keyed on `is_turn_busy`, sits in `switch_mode` above the branch. (The docstring's `cols`/`rows` are never read.)                                                  | Standardized transport switch, and the ONLY way `pty_mode` goes back to `False`. `cli` kills PTY + `visible=False`+`pty_mode=False`; `interactive` spawns PTY + `visible=True`+`pty_mode=True`. Same session. Driven from the UI in both directions since FLOWPAD-2105.                                                                                                                                                                                                   |
| `restart`                | POST | `http_restart` → `exit` + `start_pty(retry=True)`                                   | both                  | None (carries `retry=True`, so `start_failure` never blocks it). No mid-turn guard.                                                                        | exit() + start\_pty(); shell entity reused.                                                                                                                                                                                                                                                                                                                   |
...
| `drain-queue`            | POST | `_drain_queue_action` → `_schedule_queue_drain("ui")`                              | both                  | —                                                                                                                                                          | Kick a drain without adding a prompt (agent auto-launch seeds the queue server-side, the UI kicks after embedding the vibe persona). No-op when empty or busy.                                                                                                                                                                                                 |
...
| `cancel-prompt`          | POST | `_http_cancel_prompt` → worker `close_session`                                      | headless (print-mode) | Fails if no in-flight worker.                                                                                                                              | Cancel the in-flight print-mode turn — vendor graceful channel first (claude `control_request/interrupt` on stdin, codex SIGINT), SIGTERM→grace→SIGKILL as backstop. Writes the durable abort sidecar marker only when the worker was force-killed (`cancelled_gracefully=False`) — a graceful cancel leaves the vendor's own abort record in its transcript. |
...
| `load-embedded-skill`    | POST | `load_embedded_skill_action` → symlink into skills root + record in `embedded_asset_refs` | both                  | param `asset_ref` required; folder + `SKILL.md` must exist.                                                                                                | Make a skill folder discoverable to the worker (live symlink). The ref is what mounts the assets dir — without it the symlink is invisible to the worker.                                                                                                                                                                                                      |
...
| `input-dir`              | GET  | `get_input_dir`                                                                     | both                  | —                                                                                                                                                          | Absolute path of the process's input dir — `execution/input/`, the one folder `run(input=…)` saves into and `resolved_add_dirs` mounts (created if missing) + compute-node id. |
...
* `serializeAgenticContext(ctx)` — camelCase → snake\_case for the REST body (`permissionMode`→`permission_mode`, `targetVfsPath`→`target_typeid_str`, etc.).

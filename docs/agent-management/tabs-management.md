...
- `Tab` is the strip membership, label, order, project, and close/rename action
  target. A terminal tab is just a `TabRow` whose `target_type` is `shell` or
  `agentic_process`.
...
- `UnifiedTabStrip` renders the chips. Active chip state is URL-first:
  `currentDock.tabHash`.
- `TabbedTerminal` renders terminal panels only. It filters terminal `TabRow`s
...
| --- | --- | --- |
| Default terminal view | `/dock/shell` | `loadShellRoute()` resolves a default terminal `TabRow` with `loadNextProcess()` and redirects to a concrete shell/process pointer. |
| New plain terminal | `/dock/shell/new_terminal` | Creates a new `Shell`, then replaces the URL with `/dock/shell/shell-<shellId>`. |
| Plain shell tab | `/dock/shell/shell-<shellId>` or `/dock/shell/<shellId>` | `loadShell(shellId)` loads the Shell, resolves project/workdir, starts or reattaches the PTY, and clears process context. |
| Worker terminal tab | `/dock/shell/agentic_process-<processId>` | `loadProcess(processId)` loads the process, resolves project/workdir, calls `process.start({ visible: true })`, resolves the linked Shell, and sets process context. |
...
```ts
void Tab.getFromDockPointer(dock);
```

That call resolves the target entity and project from the dock itself, then
posts `Tab.newTab(pointerJson, ...)`. The backend deduplicates by `pointer`,
keeps global `tab_order`, and broadcasts `tabs_changed`; `all-tabs-store`
refreshes with `Tab.listAll()`.

Terminal tabs therefore appear because the user navigated to a terminal dock,
not because `TabbedTerminal` inserted a row.
...
- Reads rows through `useTerminalTabRows()`, which filters the one global
  `TabRow[]` list to `shell` and `agentic_process` targets.
...
4. The loader materializes the `Tab`.
...
All strip actions resolve a `TabRow` and call the `tab` action by Tab id:

- close: `Tab.closeById(tab.id)`
...
The candidate list is built from terminal `TabRow`s via `Tab.listAll()` and
`useTabs.ts` filtering, then `resolveNextTabRow()` applies:

1. pending intent
2. `TabRow.last_active_at`
3. `tab_order`

Current caveat: terminal loaders still call `Shell.activate()` /
`AgenticProcess.activate()` instead of `Tab.activateById()`. Because
`resolveNextTabRow()` reads recency from `TabRow.last_active_at`, recency is not

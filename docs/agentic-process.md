...
`unknown`, never `logged_out`). The link(+code) sign-in that turns a
`logged_out` harness into `logged_in` is driven through the `Capability` entity
— see the device-login section in `capabilities.md`.

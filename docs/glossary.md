...
* **`Inbox`** — ours. The per-owner projection of a `MessageDriver`'s threads into `Conversation`s (`flow_sdk/inbox/projection.py`): `/dock/inbox` is the local user's, `/dock/agent/<id>/inbox` an Agent's. `blocks.Inbox` is the SDK handle for one address of one `MessageDriver`, not the projection; `blocks.FolderChanges` is its object-shaped sibling (change pages of a watched folder). The **attached-channels line** on each inbox is the owner's `MessageDriver`s — `DataSource.find_owned(owner)` ∩ spec `sends` — as round marks with a status dot; a mark filters the list to that channel, the details popover holds the pause/resume switch (`status`) and delete, `+` creates a source born with that `owner`.
...
* **`LLMSource`** — ours, and the FIFTH thing this tree calls a "source", so read it precisely.
  ...
  **api_key**, or a hub **endpoint**. It is NOT `DataSource` (a system of record we ingest from),
  NOT `MessageDriver` (a `DataSource` that can also reply), NOT `SourceItem` (a record one produces), and — the collision that actually bites — NOT the hub's

...
* **`Inbox`** — ours. The per-owner projection of a `MessageSource`'s threads into `Conversation`s (`flow_sdk/inbox/projection.py`): `/dock/inbox` is the local user's, `/dock/agent/<id>/inbox` an Agent's. `blocks.Inbox` is the SDK handle for one address of one `MessageSource`, not the projection.

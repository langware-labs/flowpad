...
- **`DataDriver` is the ingestion entity, not the trace enum.** `DataDriver`
  ...
  entry, not a configured feed. The record a DataDriver produces is a `SourceItem`, generic
...
* **`MessageBlock`** — ours. The process-local prompt/reply block (`flow_sdk/blocks/message_block.py`): `send(prompt)` waits while one `listen()` consumer handles the corresponding `MessageRequest` DataSpec and calls `reply(text)`; the source owns the one-shot correlation. `Agent.respond_to(source)` owns that loop and the private per-thread process runner. It has no address or rows and is not a `DataDriver`, inbox, provider connection, `MessageSpec`, or `FlowMessage`.
* **`MessageDriver`** — ours. A bidirectional `DataDriver`: one with `channel` set whose driver declares `sends=True` and carries an identity (`account_identities`) on that channel — Slack, Telegram, a mailbox. It ingests like any data source and can reply, and it belongs to one owner (the local user or an Agent). Not the block above, not a `MessageSpec`, not a connection.
* **`Help desk`** — ours, on the hub. A desk is a hub Project with `helpdesk.enabled`; a **ticket** is a hub Conversation of `kind=helpdesk` a guest opens against it (`start_guest_conversation`), which staff `pickup` to join, and whose replies the hub masks to the desk's `display_name`. In the app the desk is a **MessageDriver**: the `helpdesk` data source (its asset folder `agentic-assets/data_driver/helpdesk/`) lists the pool as its segments, a ticket's hub messages as its records, and `send` = pickup + `add_message`. Every record carries the hub's own ids as adoption hints (`SourceItemSpec.conversation_id` / `message_id`), so the projection lands on the hub-mirrored rows instead of minting twins. An Agent that owns a desk answers everyone (`open_inbound`) unless its allowlist names someone. The requester side stays the hub-mirrored conversation on the portal.
* **`Inbox`** — ours. The per-owner projection of a `MessageDriver`'s threads into `Conversation`s (`flow_sdk/inbox/projection.py`): `/dock/inbox` is the local user's, `/dock/agent/<id>/inbox` an Agent's. `blocks.Inbox` is the SDK handle for one address of one `MessageDriver`, not the projection; `blocks.FolderChanges` is its object-shaped sibling (change pages of a watched folder). The **attached-channels line** on each inbox is the owner's `MessageDriver`s — `DataDriver.find_owned(owner)` ∩ spec `sends` — as round marks with a status dot; a mark filters the list to that channel, the details popover holds the pause/resume switch (`status`) and delete, `+` creates a source born with that `owner`.
* **`owner`** — ours. Whose row it is: a user or Agent `TypeId` on `DataDriver`, `MessageThread` and `Conversation`, defaulting to the local user. THE key the inbox partitions by — an Agent's inbox is `owner == agent`, a filter, not a walk from one provider. Read only through `inbox.projection.owner_of`, which also resolves rows written before the field existed (`config.agent_id` → that agent). Not `created_by` (the hub's creator mirror) and not the roster's `owner` role (hub-side authz).
* **`ManifestSpec`** — ours. The shape of a data source's `data_driver.json` and the `asset_spec` of the `DataDriverSpec` folder asset; every authoring rule is a validator on it.
* **driver / source** — ours, the split the `DataSource` → `DataDriver` rename draws. A **driver** is the code and manifest that knows how to talk to a system: `DriverType` (`flow_sdk/ingest/driver_types.py`), the driver registry (`driver_registry.py`), and the asset family `agentic-assets/data_driver/<name>/data_driver.json`. A **source** is the configured, running connection and what it produces: `Source` (the live session), `SourceBinding`, `SourceItem`, `SourceStatus`, `SourceHealth` — the `flow_sdk/sources/` authoring contract keeps the source vocabulary because every authored `source.py` imports it. The configured row is `DataDriver` (type `data_driver`); the product still says **Data sources** (`/dock/data-sources`, `flow source`, the webhook `/api/v1/data_source/webhook/<name>` providers hold). Pre-rename folders (`data_source/`, `data_source.json`) are retired forms the scan reports until `migration_2026_09_retired_asset_forms` moves them. A later type rename reuses `flow_sdk/migrations/entity_type_rename.py`.
* **`LLMSource`** — ours, and the FIFTH thing this tree calls a "source", so read it precisely.
  ...
  **api_key**, or a hub **endpoint**. It is NOT `DataDriver` (a system of record we ingest from),
  NOT `MessageDriver` (a `DataDriver` that can also reply), NOT `SourceItem` (a record one produces), and — the collision that actually bites — NOT the hub's
...
* **`SecretStore`** — ours (was *value store*). A place secret values live, keyed by environment variable name: a type plus its config (`flow_sdk/secrets`). Three ship: `env_file` (a dotenv file; a credential's `value_store` spells it `env`, the scope root's `.env.local` by default), `vault` (the per-instance encrypted store, `sodot` on disk) and `gcp_secret_manager` (a Google Cloud Secret Manager project, read through a bound `google` connection). A credential names its store per environment; a `DataDriver` instance binds one (`set_secret_store`). Not a `Connection`, which is an account that hands out a token. See [secret-stores](snippets/secret-stores.md).
...
| `PromptCompletion` | `flow_sdk/builtin/prompt_completion.py` | The per-turn result of a live session; rides the reply as a `prompt_completion-<id>` TYPE_ID attachment with `result_preview`. Never a runnable prompt (no Claude↔Claude loop). |
| `ContactPermission` (standing grant) | `flow_sdk/builtin/contact_permission.py` | The host's LOCAL policy that a contact's NEW sessions start approved (`auto_approve_session`), per project or everywhere. Not consent for anything else — pause / reply policy / disconnect live on the session. |

...
* **`Inbox`** — ours. The per-owner projection of a `MessageSource`'s threads into `Conversation`s (`flow_sdk/inbox/projection.py`): `/dock/inbox` is the local user's, `/dock/agent/<id>/inbox` an Agent's. `blocks.Inbox` is the SDK handle for one address of one `MessageSource`, not the projection. The **attached-channels bar** on each inbox is the owner's `MessageSource`s — `DataSource.find_owned(owner)` ∩ spec `sends` — as Dock-style icons: click pauses/resumes the source (`status`), a parked one opens Data Sources; `+` creates a source born with that `owner`.

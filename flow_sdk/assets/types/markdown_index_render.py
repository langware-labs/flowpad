...
from flow_sdk.assets.frontmatter import _atomic_write_text, _render_frontmatter
...
def write_pair(folder: Path, data: IndexMdJson, json_path: Path | None = None) -> tuple[Path, Path]:
    """Write canonical JSON and rendered markdown, each with atomic replacement.
    ...
    _atomic_write_text(json_path, data.model_dump_json(indent=2))

    md_path.parent.mkdir(parents=True, exist_ok=True)
    _atomic_write_text(md_path, render_index_md(data))
...
def _cli(argv: list[str] | None = None) -> int:
    ...
    if not args:
        print("usage: render <input.json> [<output.md>]", file=sys.stderr)  # noqa: T201 -- explicit module CLI diagnostics
        return 2
    json_path = Path(args[0])
    data = load_index_md_json(json_path)
    if data is None:
        print(f"error: cannot read {json_path}", file=sys.stderr)  # noqa: T201 -- explicit module CLI diagnostics
        return 1
    md = render_index_md(data)
    if len(args) >= 2:
        _atomic_write_text(Path(args[1]), md)
    else:
        sys.stdout.write(md)
    return 0

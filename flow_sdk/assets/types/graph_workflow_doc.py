"""The flow document — ``graph.json``, the semantic truth of an GraphWorkflow.

An GraphWorkflow folder holds:

    .claude/graph-workflows/<name>/
...
* ``GraphWorkflowNodeDef`` — ``node_type`` is one of:
    ...
    - ``function`` — a GraphWorkflowFunction (``node_data.function`` = registry name or
                     ``scripts/<file>.py``; ``node_data.runtime`` = ``inline`` |
                     ``subprocess``). One contract everywhere:
                     ``on_graph_workflow_event(event_name, data, flow_ctx)``.
* ``GraphWorkflowEdgeDef`` — ``{from: {node, event}, to: {node}}``. Events are LOCAL to
...
def retired_node_shape(node: dict) -> str | None:
    ...
    raw node dict on a pre-GraphWorkflowFunction shape, or None. The parse validator
...
class GraphWorkflowSubscriptionDef(BaseModel):
...
class GraphWorkflowEdgeDef(BaseModel):
    id: str
...
class GraphWorkflowDoc(DataSpec):
    # Journey extensions and provider-authored metadata travel with the document.
    model_config = {"extra": "allow"}

    version: int = 1
    id: Optional[str] = None  # the GraphWorkflow entity id (adopted by gen_uuid)
    name: str = ""
    description: str = ""
    enabled: bool = True  # the flow's active switch
    config: GraphWorkflowConfig = Field(default_factory=GraphWorkflowConfig)
    subscriptions: list[GraphWorkflowSubscriptionDef] = Field(default_factory=list)
    nodes: list[GraphWorkflowNodeDef] = Field(default_factory=list)
    edges: list[GraphWorkflowEdgeDef] = Field(default_factory=list)
    ...
    def node(self, node_id: str) -> Optional[GraphWorkflowNodeDef]:
        return next((n for n in self.nodes if n.id == node_id), None)

    def targets_for(self, source_node: str, event: str) -> list[GraphWorkflowNodeDef]:
        """Edge routing: exact event match or catch-all, within this flow."""
        out: list[GraphWorkflowNodeDef] = []
        for e in self.edges:
    ...
    def trigger_nodes(self) -> list[GraphWorkflowNodeDef]:
    ...
    def trigger_nodes_for(self, trigger_id: str) -> list[GraphWorkflowNodeDef]:
...
def _parse_entity_ref(node: GraphWorkflowNodeDef, expected_type: str) -> str | None:
...
def _parse_trigger_ref(node: GraphWorkflowNodeDef) -> str | None:
...
def parse_graph_workflow_doc(text: str) -> GraphWorkflowDoc:
    ...
    return GraphWorkflowDoc.model_validate(payload)

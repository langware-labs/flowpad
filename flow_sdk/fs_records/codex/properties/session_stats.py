...
Mirror of ``flow_sdk.fs_records.claude.properties.session_stats`` but for the
...
def _get_codex_session_batch_stats(instance: "Record") -> dict:
...
class _CodexSessionStatsProp(PropertyRecord):
    """PropertyRecord descriptor backed by the Codex batch stats cache."""

    _record_type: ClassVar[str] = "prop_codex_session_stat"
    _default_ttl: ClassVar[float] = -1
    ...
    def run_discovery(self, instance: "Record", force: bool = False) -> Any:
        if force:
            try:
                object.__delattr__(instance, "_codex_session_batch_stats")
            except AttributeError:
                pass
        return _get_codex_session_batch_stats(instance).get(self._field, self._default)

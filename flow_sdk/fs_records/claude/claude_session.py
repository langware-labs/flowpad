...
class ClaudeSessionRecord(Record):
    ...
    def _parse_fts(self) -> tuple[str | None, str | None]:
        ...
        title   = latest ``ai-title`` (Claude auto-titler) if present, otherwise
                  the latest ``custom-title`` (user /rename); truncated to 120 chars
        ...
        try:
            ...
            object.__setattr__(self, "_fts_cache", result)
            return result
        chosen = last_ai_title or last_custom_title
        title = chosen[:120] if chosen else None
        result = (title, "\n".join(lines) or None)
        object.__setattr__(self, "_fts_cache", result)
        return result
    ...
    def from_jsonl(cls, path: str | Path) -> Self:
        ...
        try:
            ...
            # Walk tail in reverse; ai-title wins over custom-title.
            tail_custom: str = ""

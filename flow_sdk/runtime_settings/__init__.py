...
from .dev_settings import DevRuntimeSettings
from .test_settings import TestRuntimeSettings

ENV_FLOW_RUNTIME_NAME = "FLOW_RUNTIME"
ENV_FLOWPAD_DEV_RUNTIME_NAME = "FLOWPAD_DEV"
...
_RUNTIMES: dict[tuple[str, Path], BaseRuntimeSettings] = {}
...
def get_runtime_settings() -> BaseRuntimeSettings:
    ...
    """
    name = _resolve_runtime_name_from_env()
    ...
    if name == "test":
        runtime = TestRuntimeSettings.from_env()
    elif name == "dev":
        runtime = DevRuntimeSettings.from_env()
    else:
        runtime = BaseRuntimeSettings.from_env(name=name)
...
_reset_for_tests = reset_runtime_settings
...
def override_db_path(new_path: "Path | str") -> None:
    ...
    **Drops the cached SQLite driver** so the next ``get_db_driver()``
    ...
    flow_home = _resolve_flow_home_from_env()
    key = (name, flow_home)
    current = _RUNTIMES.get(key) or get_runtime_settings()
    _RUNTIMES[key] = _replace(current, db_path=Path(new_path))
...
def override_records_root(new_path: "Path | str") -> None:
    ...
    key = (name, flow_home)
    current = _RUNTIMES.get(key) or get_runtime_settings()
    _RUNTIMES[key] = _replace(current, records_root=Path(new_path))


def _resolve_runtime_name_from_env() -> str:
    ...
    """
    explicit = os.environ.get(ENV_FLOW_RUNTIME_NAME)
    if explicit:
        return explicit
    if (os.environ.get(ENV_FLOWPAD_TEST, "").lower() == "true"
        ...
        return "test"
    if os.environ.get(ENV_FLOWPAD_DEV_RUNTIME_NAME, "").lower() == "true":
...
def _resolve_flow_home_from_env() -> Path:
    return BaseRuntimeSettings._resolve_flow_home()
...
def _is_test_mode() -> bool:
    return _resolve_runtime_name_from_env() == "test"


def _is_dev_mode() -> bool:
    return _resolve_runtime_name_from_env() == "dev"

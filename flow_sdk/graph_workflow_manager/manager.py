"""GraphWorkflowManager v2 — runs GraphWorkflow documents.

Model: an GraphWorkflow is a folder document (``graph.json``); events are LOCAL
...
* ``function`` — a GraphWorkflowFunction (``on_graph_workflow_event(event_name, data, flow_ctx)``)
...
* the RUN itself: ``records_root/graph_workflow_run/<stem>/execution/{input,output}``
...
RUN_RECORD_TYPE = "graph_workflow_run"
...
def prepare_execution_io(base: Path, fe: "RunEvent") -> tuple[Path, Path]:
    ...
    try:
        ...
        logger.debug("GraphWorkflowManager: execution input record failed", exc_info=True)
...
class _LoadedFlow:
    ...
    def __init__(self, flow_id: str, folder: Path, doc: GraphWorkflowDoc, mtime: float) -> None:
...
class _NodeRuntime:
    ...
    def __init__(self) -> None:
        ...
        self.queue: "deque[tuple[RunEvent, GraphWorkflowNodeDef, _Run]]" = deque()
        self.active = 0
...
class InlineGraphWorkflowCtx:
    ...
    async def post(self, path: str, body: dict, *, timeout: int = 60) -> dict:
        ...
        from flow_sdk.graph_workflow_manager.function_runner import GraphWorkflowCtx as _SubCtx
...
# enabled / graph.json) driven by this engine. GraphWorkflow is the native type;
...
class GraphWorkflowManager:
    ...
    async def load_flow(self, flow_id: str, entity: GraphWorkflow | None = None) -> Optional[_LoadedFlow]:
        if entity is None:
        ...
        if cached is not None and cached.mtime == mtime:
            return cached
        try:
            doc = parse_graph_workflow_doc(graph.read_text(encoding="utf-8"))
        except ValueError as e:
            logger.warning("GraphWorkflowManager: bad graph.json for %s: %s", flow_id, e)
            return None
    ...
    def _subscription_handler(self, flow_id: str, sub):
        ...
        async def _on_event(event: "FlowEvent") -> None:
            ...
            if target_of("graph_workflow", flow_id) in event.ctx.scope:
                return
    ...
    async def arm_all_flow_subscriptions(self) -> None:
        ...
        for entity in await GraphWorkflow.get_all({}):
            try:
                ...
                logger.exception("GraphWorkflowManager: boot subscription arm failed for %s", entity.id)
        if self._rearm_unsub is None:
            # Bus dogfooding: an GraphWorkflow save re-loads + re-arms that flow.
    ...
    async def _start_run(self, flow: _LoadedFlow) -> _Run:
        ...
        try:
            ...
            logger.exception("GraphWorkflowManager: run record dir create failed for %s", run.id)
    ...
    def _record_run_event(self, run: _Run, fe: RunEvent, direction: str,
        ...
        try:
            ...
            logger.debug("GraphWorkflowManager: run %s record failed", direction, exc_info=True)
        if direction == "output":
    ...
    async def _deliver(self, run: _Run, node: GraphWorkflowNodeDef, fe: RunEvent) -> None:
    ...
    def _limit_for(self, node: GraphWorkflowNodeDef) -> int:
        nd = node.node_data
        if nd.get("execution_mode") == "parallel":
            return max(1, int(nd.get("parallel_limit") or 1))
        return 1

    async def _drain(self, flow_id: str, node: GraphWorkflowNodeDef) -> None:
    ...
    async def _run_function_inline(self, run: _Run, node: GraphWorkflowNodeDef, fe: RunEvent,
        ...
        fn = graph_workflow_functions.get(ref)
        if fn is None:
            self._finish_execution(run, rt)
            raise RuntimeError(f"No registered GraphWorkflowFunction {ref!r}")
        exec_dir = inline_exec_dir(run.id, seq, node.id)
    ...
    async def _attach_to_run(self, run: _Run, proc: Any) -> None:
        try:
            ...
            row = await GraphWorkflowRun.get_by_id(run.id)
            if row is not None:
                await row.attach_child(proc)
        except Exception:
            logger.debug("GraphWorkflowManager: run→process attach failed", exc_info=True)
            ...
            while True:
                ...
                if busy:
                    ...
                    try:
                        ...
                        logger.exception("GraphWorkflowManager: one-shot exit failed for %s", proc_id)
                    break
        except Exception:
            logger.exception("GraphWorkflowManager: agent watcher failed for node %s", node.id)
    ...
    async def _agent_output(proc: Any) -> str:
        ...
        try:
            ...
            logger.debug("GraphWorkflowManager: agent output read failed", exc_info=True)
        return ""
    ...
    def _trip(self, run: _Run, reason: str) -> None:
        ...
        logger.warning("GraphWorkflowManager: run %s tripped: %s", run.id, reason)
    ...
    async def _finalize(self, run: _Run) -> None:
        ...
        try:
            ...
            logger.exception("GraphWorkflowManager: run retention prune failed for %s", run.flow.flow_id)
    ...
    async def _prune_runs(self, flow: _LoadedFlow) -> None:
        ...
        rows = await GraphWorkflowRun.get_all({"flow_id": flow.flow_id})
            ...
            try:
                for child in await row.get_children():
                    try:
                        ...
                        logger.debug("GraphWorkflowManager: prune child destroy failed", exc_info=True)
            except Exception:
                logger.debug("GraphWorkflowManager: prune children listing failed", exc_info=True)
...
_manager: Optional[GraphWorkflowManager] = None


def get_graph_workflow_manager() -> GraphWorkflowManager:
    global _manager
    if _manager is None:
        _manager = GraphWorkflowManager()
    return _manager

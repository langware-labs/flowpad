"""GraphWorkflowFunction subprocess runner — executes a function node in ISOLATION.

The one flow-function contract (identical for inline and subprocess runtimes):

    def on_graph_workflow_event(event_name: str, data: dict, flow_ctx) -> dict | None:
        ...
...
or a ``graph_workflow_functions`` registry name (the subprocess imports flow_sdk and
...
Isolation: runs via ``python -m flow_sdk.graph_workflow_manager.function_runner`` in its
...
# ── subprocess side (python -m flow_sdk.graph_workflow_manager.function_runner <ref>) ──


class GraphWorkflowCtx:
    """What ``on_graph_workflow_event`` receives as ``flow_ctx`` (subprocess runtime)."""
    ...
    def emit_flow_event(self, key: str, val: Any = None) -> None:
        ...
        self.post(
            f"/api/v1/graph-workflows/{self.flow_id}/inject",
            {
                "event": key,
                "data": data,
...
def _resolve_handler(target: str):
    """Script path → import the file; anything else → graph_workflow_functions registry."""
    if target.endswith(".py"):
        ...
        handler = getattr(module, "on_graph_workflow_event", None)
    ...
    if handler is None:
        print(f"no registered GraphWorkflowFunction {target!r}", file=sys.stderr)
        return None
    return handler
...
def _main() -> int:
    if len(sys.argv) != 2:
        print("usage: python -m flow_sdk.graph_workflow_manager.function_runner <script.py | registry-name>",
              file=sys.stderr)
        return 64
    payload = json.loads(sys.stdin.read() or "{}")
    ctx_raw = payload.get("ctx") or {}
    ctx = GraphWorkflowCtx(

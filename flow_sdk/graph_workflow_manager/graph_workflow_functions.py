"""GraphWorkflowFunction registry — the flow system's built-in function library.

A **GraphWorkflowFunction** is a named Python function with the one flow contract:

    @graph_workflow_functions.register("publish_report", meaning="post a report to the feed")
    async def on_graph_workflow_event(event_name: str, data: dict, flow_ctx) -> dict | None:
        ...
...
def register(name: str, meaning: Optional[str] = None):
    """Decorator: register a GraphWorkflowFunction under ``name`` (re-registration replaces)."""
...
def get(name: str) -> Optional[Callable[..., Any]]:
    """Look up a GraphWorkflowFunction by name. Returns None if not registered."""

...
rides in the message bundle (``fs_origins.json``) and stamps received entities'
...
def as_git(origin: Any) -> "Optional[GitOrigin]":
    """Narrow an entity's ``origin`` to the git kind — the read seam for every
    clone/branch site. ``None`` for absent, non-git or malformed."""
    from flow_sdk.fs_store.origin.field import ORIGIN_ADAPTER  # noqa: PLC0415

    typed = origin if isinstance(origin, GitOrigin) else ORIGIN_ADAPTER.validate_python(origin)
    return typed if isinstance(typed, GitOrigin) else None
...
class GitOrigin(FSOrigin):
    ...
    def next_clone_target(self) -> Path:
        """Where a checkout of this origin belongs in the local workspace, when
        the repo IS the thing's identity: the folder leaf derives from the clone
        URL, and an existing ``<leaf>``/``<leaf>-N`` that already matches this
        origin (remote + branch) is reused rather than cloned twice.

        The other placement policy is :func:`fresh_clone_slot` — never reuse.
        """
        from flow_sdk.utils.git import derive_repo_leaf_from_url  # noqa: PLC0415

        leaf = derive_repo_leaf_from_url(self.clone_url()) or (self.name or "").removesuffix(".git")
        return _slot(leaf, "repo", accept=lambda p: self.matches_checkout(p, require_branch=True))

    def local_checkout(self) -> Path | None:
        """The local clone of this origin's URL on its pinned branch, if the
        machine has one (a clone on another branch is not this checkout)."""
        from flow_sdk.utils.git import find_local_repo_for_url  # noqa: PLC0415

        local = find_local_repo_for_url(self.clone_url())
        return Path(local) if local and self.matches_checkout(Path(local), require_branch=True) else None
...
def has_content(path: Path) -> bool:
    """True for a file, or a directory with at least one entry — an EMPTY
    directory has nothing to lose and ``git clone`` writes into it happily."""
...
def _sanitize_leaf(leaf: str, default: str) -> str:
...
def _slot(leaf: str, default: str, *, accept) -> Path:
    ...
    candidate = base / leaf
    n = 2
    while candidate.exists() and not accept(candidate):
...
def fresh_clone_slot(preferred_leaf: str, *, reuse_empty: bool = True) -> Path:
    ...
    reuses an existing checkout, it suffixes past a collision.

    ``reuse_empty``: an EMPTY directory is not a collision. This matters
    because a Project reserves ``<workspace>/<name>`` when it is constructed,
    so treating that as taken would push every engagement to ``<name>-2`` and
    strand the reserved directory as an empty stray. A caller that wants a

...
class OpenAIDriver(LLMDriver):
    ...
    def supported_models(cls) -> List[LLMModelInfo]:
        return [
            LLMModelInfo(
                name="gpt-4o-2024-11-20", max_input_tokens=128000, max_output_tokens=2**14, provider=cls.provider
            ),
            LLMModelInfo(name="gpt-4o", max_input_tokens=128000, max_output_tokens=2**14, provider=cls.provider),
            LLMModelInfo(
                name="gpt-4o-mini-2024-07-18", max_input_tokens=128000, max_output_tokens=2**14, provider=cls.provider
            ),
            LLMModelInfo(name="gpt-4o-mini", max_input_tokens=128000, max_output_tokens=2**14, provider=cls.provider),
            LLMModelInfo(name="gpt-5-nano", max_input_tokens=400_000, max_output_tokens=128000, provider=cls.provider),
        ]
    ...
    async def generate(
        ...
        try:
            ...
            response_stream = await self.client.chat.completions.create(
                ...
                temperature=temperature if not model.endswith("-nano") else 1,
                stop=stop,
                stream=True,

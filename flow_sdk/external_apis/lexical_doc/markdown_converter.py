"""
Markdown conversion utilities for Lexical documents.

This module provides bidirectional conversion between Markdown and Lexical formats.
"""
...
def process_text_node_to_md(node: dict) -> str:
    text = node.get("text", "")
...
def process_paragraph_node_to_md(paragraph: dict) -> str:
    paragraph_text = "".join([process_child_to_md(child) for child in paragraph.get("children", [])])
...
def process_fence_node_to_md(fence: dict) -> str:
    fence_info = fence.get("info", "")
    fence_content = json.dumps(fence["content"]) if "content" in fence else ""
    return f"```{fence_info}\n{fence_content}\n```"


def process_listitem_node_to_md(list_item: dict) -> str:
    # Process each child within the list item
    item_text = "".join([process_child_to_md(child) for child in list_item.get("children", [])])
    return item_text


def process_list_node_to_md(list_node: dict) -> str:
    ...
    for index, item in enumerate(list_node.get("children", []), start=1):
        item_text = process_child_to_md(item)
        checked = item.get("checked", False)  # Default to False if "checked" is missing
...
def process_table_cell_to_md(table_cell: dict) -> str:
    return "\n\n".join([process_child_to_md(child) for child in table_cell.get("children", [])])


def process_table_row_to_md(table_row: dict) -> str:
    return "| " + " | ".join([process_child_to_md(cell) for cell in table_row.get("children", [])]) + " |"


def process_table_node_to_md(table_node: dict) -> str:
    table_markdown = []
    for row in table_node.get("children", []):
        table_markdown.append(process_child_to_md(row))
...
def process_heading_node_to_md(heading_node: dict) -> str:
    heading_text = "".join([process_child_to_md(child) for child in heading_node.get("children", [])])
    heading_tag = heading_node.get("tag", "h1")
    heading_level = int(heading_tag[1])
    return f"{'#' * heading_level} {heading_text}"


def process_linebreak_node_to_md(heading_node: dict) -> str:
    return "\n"


def process_horizontalrule_node_to_md(heading_node: dict) -> str:
    return "---"


def process_quote_node_to_md(quote_node: dict) -> str:
    quote_text = "".join([process_child_to_md(child) for child in quote_node.get("children", [])])
    return f"> {quote_text}"


def process_hashtag_node_to_md(hashtag_node: dict) -> str:
    return f"#{hashtag_node.get('text', '')}"


def process_link_node_to_md(link_node: dict) -> str:
    url = link_node.get("url", "")
    link_text = "".join([process_child_to_md(child) for child in link_node.get("children", [])])
    return f"[{link_text}]({url})"


def process_emoji_node_to_md(emoji_node: dict) -> str:
    return emoji_node.get("emoji", "")


def process_collapsible_node_to_md(collapsible_node: dict):
    ...
    for collapsible_child in collapsible_children:
        if collapsible_child.get("type") == "collapsible-title":
            title = "".join([process_child_to_md(child) for child in collapsible_child.get("children", [])])
        elif collapsible_child.get("type") == "collapsible-content":
            content = "".join([process_child_to_md(child) for child in collapsible_child.get("children", [])])
        else:
...
def process_hinted_node_to_md(hinted_node: dict):
    ...
    for hinted_child in hinted_children:
        if hinted_child.get("type") == "hint":
            hint = "".join([process_child_to_md(child) for child in hinted_child.get("children", [])])
        elif hinted_child.get("type") == "hinted-content":
            content = "".join([process_child_to_md(child) for child in hinted_child.get("children", [])])
        else:
...
def process_child_to_md(child: dict) -> str:
    if child.get("type") == "text":
        return process_text_node_to_md(child)
    elif child.get("type") == "paragraph":
        return process_paragraph_node_to_md(child)
    elif child.get("type") == "fence":
        return process_fence_node_to_md(child)
    elif child.get("type") == "list":
        return process_list_node_to_md(child)
    elif child.get("type") == "listitem":
        return process_listitem_node_to_md(child)
    elif child.get("type") == "table":
        return process_table_node_to_md(child)
    elif child.get("type") == "tablerow":
        return process_table_row_to_md(child)
    elif child.get("type") == "tablecell":
        return process_table_cell_to_md(child)
    elif child.get("type") == "heading":
        return process_heading_node_to_md(child)
    elif child.get("type") == "linebreak":
        return process_linebreak_node_to_md(child)
    elif child.get("type") == "horizontalrule":
        return process_horizontalrule_node_to_md(child)
    elif child.get("type") == "quote":
        return process_quote_node_to_md(child)
    elif child.get("type") == "hashtag":
        return process_hashtag_node_to_md(child)
    elif child.get("type") == "link":
        return process_link_node_to_md(child)
    elif child.get("type") == "emoji":
        return process_emoji_node_to_md(child)
    elif child.get("type") == "collapsible-container":
        return process_collapsible_node_to_md(child)
    elif child.get("type") == "hinted":
        return process_hinted_node_to_md(child)
...
def lexical_to_markdown(data: dict) -> str:
    """Convert a Lexical document to Markdown format."""
    root = data.get("root", {})
    return "\n\n".join([process_child_to_md(child) for child in root.get("children", [])])

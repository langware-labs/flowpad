"""
Utility functions for working with Lexical documents.
"""
...
def merge_lexicals(first: LexicalRoot, second: LexicalRoot) -> LexicalRoot:
    """
    Merge two LexicalRoot documents together.

    Args:
        first: The first LexicalRoot document
        second: The second LexicalRoot document to append

    Returns:
        A new LexicalRoot with children from both documents

    Raises:
        ValueError: If either document doesn't have proper root structure
    """
...
def json_to_lexical(json_content: Any) -> LexicalRoot:
    """
    Convert JSON content to a LexicalRoot document.

    Args:
        json_content: Any JSON-serializable content

    Returns:
        A LexicalRoot document
    """
    from .markdown_converter import markdown_to_lexical
...
def single_func_lexical(func_title: str, func_content: Any) -> LexicalRoot:
    """
    Create a LexicalRoot containing a single function/code block.

    Args:
        func_title: Title for the code block
        func_content: Content to put in the code block

    Returns:
        A LexicalRoot with a single fence node
    """
    from .markdown_converter import markdown_to_lexical
...
def add_func_to_lexical(lexical: LexicalRoot, func_title: str, func_content: Any) -> LexicalRoot:
    """
    Add a function/code block to an existing LexicalRoot.

    Args:
        lexical: Existing LexicalRoot document
        func_title: Title for the new code block
        func_content: Content for the new code block

    Returns:
        A new LexicalRoot with the added function
    """
...
def create_root(children: list[LexicalNode] = []) -> LexicalRoot:
    """Create a LexicalRoot with the given children."""
...
def get_fence_nodes(lexical: LexicalRoot) -> list[LexicalFenceNode]:
    """Get all fence nodes recursively from a LexicalRoot document."""

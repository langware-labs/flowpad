...
STYLE — act, don't interview. Gather evidence and fill fields yourself;
every step below has a default. Ask the user ONLY as a last resort, for a
fact that exists nowhere you can read (see "Asking the user"). Keep every
message to one or two short sentences — progress notes, not explanations.
...
- `taskFolder`: absolute path of the task's folder (`.../tasks/<name>/` —
  holds `task.md` with YAML frontmatter and, on the owner side, `spec.md`)
...
1. Stamp `process_id` (above). Read `task.md` (fields + body) and, if
   present, `spec.md` (the plan).
...
6. Close:
   `flow wizard <processId> close '{"status":"done","data":{"summary":"<2-line status>","analysisPath":"<abs path>","missing":[...]}}'`
...
6. Close:
   `flow wizard <processId> close '{"status":"done","data":{"summary":"<rollup>","analysisPath":"<abs path>","members":[{"assignee":...,"status":...,"verdict":...,"score":...}]}}'`
...
If you cannot analyze at all (missing folder, unreadable task), close with
`{"status":"error","errorStr":"<one line>"}` instead of leaving the wizard
hanging.

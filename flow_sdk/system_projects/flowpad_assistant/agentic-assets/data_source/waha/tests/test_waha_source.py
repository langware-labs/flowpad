...
async def test_a_reply_goes_to_the_raw_chat_and_quotes_the_message_it_answers(serve, recorded):
    double, base = serve()
    outcome = await source_type("waha").send(_source(base), thread_key=LID, to=LID, text="hi", in_reply_to="true_x_AAA")
    (body,) = _sent(double)
...
async def test_the_noweb_engine_answer_still_names_the_sent_message(serve, recorded):
    ...
    outcome = await source_type("waha").send(_source(base), thread_key=CHAT, to=CHAT, text="hi")
...
async def test_a_bare_number_is_sent_to_its_phone_chat(serve, recorded):
    double, base = serve()
    await source_type("waha").send(_source(base), thread_key="", to="+972 50 123 4567", text="hello")
    (body,) = _sent(double)
...
async def test_a_new_source_creates_its_session_with_the_signed_webhook_and_asks_for_the_qr(serve):
    ...
    verdict = await source_type("waha").verify(_source(base))
...
async def test_a_session_pointed_elsewhere_is_repointed_at_this_instance(serve):
    ...
    await source_type("waha").verify(_source(base))
...
async def test_a_paired_session_is_ready_and_stamps_the_number(serve):
    _, base = serve()
    source = _source(base)
    verdict = await source_type("waha").verify(source)
...
async def test_a_wrong_api_key_says_which_key(serve):
    _, base = serve()
    verdict = await source_type("waha").verify(_source(base, api_key="wrong"))

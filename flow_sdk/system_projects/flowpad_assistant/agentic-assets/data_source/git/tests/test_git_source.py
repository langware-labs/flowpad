...
GitSource = asset_module("git").GitSource
...
async def test_the_first_pass_diffs_the_empty_tree_and_never_walks(tmp_path):
    ...
    result = await source_type("git").traverse(_row(repo), _view())
...
async def test_a_deletion_is_exact(tmp_path):
    ...
    _commit(repo)
    second = await driver.traverse(_row(repo), _view(first))
    assert [Path(t).name for t in second.tombstones] == ["b.md"] and set(second.manifest) == {"a.md"}
...
async def test_a_legacy_sha_cursor_resumes_there(tmp_path):
    ...
    _commit(repo)
    result = await source_type("git").traverse(_row(repo), _view({"sha": seed}))
...
async def test_a_directory_that_is_not_a_repository_needs_a_person(tmp_path):
    ...
    with pytest.raises(Exception) as caught:
        await source_type("git").traverse(_row(tmp_path / "plain"), _view())
...
async def test_verify_says_what_is_missing(tmp_path, setup, expected):
    verdict = await source_type("git").verify(setup(tmp_path))
...
async def test_verify_passes_for_a_repository_with_commits(tmp_path):
    assert (await source_type("git").verify(_row(_repo(tmp_path / "repo", {"a.md": "a"})))).ready is True
...
async def test_identity_is_the_documented_cross_machine_handle_when_there_is_a_remote(tmp_path):
    ...
    result = await source_type("git").traverse(_row(clone), _view())
    assert source_type("git").origin_id_for(_row(clone), result.refs[0]) not in ("", result.refs[0])


async def test_the_working_tree_is_never_stamped():
    assert source_type("git").stamps_identity is False

...
AgentSource = asset_module("agent").AgentSource
...
def test_the_connector_is_the_channel():
    assert source_type("agent").channel_for(_row()) == "gmail"
    assert source_type("agent").channel_for(_row(connector="slack")) == "slack"


async def test_the_worker_records_so_a_traversal_returns_no_items(worker):
    result = await source_type("agent").traverse(_row(), _view())
...
async def test_the_receipts_high_water_is_the_next_runs_floor(worker):
    await source_type("agent").traverse(_row(), _view({"high_water": "2026-08-31T00:00:00+00:00"}))
...
async def test_a_reported_error_classifies_by_what_fixes_it(worker, reported, health):
    ...
    with pytest.raises(Exception) as caught:
        await source_type("agent").traverse(_row(), _view())
...
async def test_a_send_carries_the_thread_the_address_and_the_local_conversation(worker):
    out = await source_type("agent").send(_row(), thread_key="t-1", to="a@b.c", text="hi", subject="Re: x", conversation_id="conv-1")
    run = worker.sends[0]
...
async def test_a_draft_is_reported_as_a_draft_and_never_recorded(worker):
    ...
    out = await source_type("agent").send(_row(), thread_key="t-1", to="a@b.c", text="hi")

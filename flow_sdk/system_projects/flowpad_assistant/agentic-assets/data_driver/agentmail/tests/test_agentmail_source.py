...
class TestMapping:
    async def test_the_providers_own_id_the_thread_and_the_sender(self, mail):
        (item,) = (await driver_type("agentmail").traverse(_row(mail), _view())).items
...
class TestCursor:
    async def test_only_what_is_newer_than_the_high_water_returns(self, mail):
        ...
        result = await driver_type("agentmail").traverse(_row(mail), _view({"cursor": AgentMailSource.resume_after("2026-08-02T00:00:00.000Z")}))
    ...
    async def test_a_legacy_high_water_is_adopted(self, mail):
        result = await driver_type("agentmail").traverse(_row(mail), _view({"high_water": MSG["timestamp"]}))
        assert result.items == [] and result.unchanged is True

    async def test_nothing_new_is_reported_unchanged(self, mail):
        mail.messages = []
        assert (await driver_type("agentmail").traverse(_row(mail), _view())).unchanged is True


class TestSend:
    async def test_a_reply_uses_the_reply_route_with_an_encoded_id(self, mail):
        out = await driver_type("agentmail").send(_row(mail), thread_key="t-1", to="joe@agentmail.to", text="hi", in_reply_to=MSG["message_id"])
    ...
    async def test_without_a_parent_it_starts_a_new_message(self, mail):
        await driver_type("agentmail").send(_row(mail), thread_key="", to="joe@agentmail.to", text="hi", subject="Hello")
    ...
    async def test_the_sent_copy_is_not_recorded_here(self, mail):
        out = await driver_type("agentmail").send(_row(mail), thread_key="", to="j@x.to", text="hi")
...
class TestTheKey:
    ...
    async def _headers(self, mail, **config):
        await driver_type("agentmail").traverse(_row(mail, **config), _view())
    ...
    async def test_the_key_comes_from_the_store_without_a_project(self, mail, monkeypatch):
        ...
        await driver_type("agentmail").traverse(_row(mail, api_key=""), _view())
    ...
    async def test_the_store_wins_over_a_legacy_config_key(self, mail, monkeypatch):
        ...
        result = await driver_type("agentmail").traverse(_row(mail, api_key="am_wrong"), _view())
    ...
    async def test_a_legacy_config_key_still_works(self, mail):
        assert (await driver_type("agentmail").traverse(_row(mail), _view())).items
    ...
    async def test_a_refused_key_needs_a_person(self, mail):
        with pytest.raises(Exception) as caught:
            await driver_type("agentmail").traverse(_row(mail, api_key="am_wrong"), _view())
        assert classify(caught.value)[0] is SourceHealth.CONFIG_ERROR

"""Contacts-group task fan-out: ``create-group-task`` + ``sync-group``.

Groups only. Handing a task to ONE person is not a group of one — it is a share,
and it lives in ``task_assign_action`` (``assign-task``): one row, no child, no
``kind`` flip, no sync. Everything here exists because N members each need their
own status.
...
# The group flow depends on the primitive; never the reverse.
from flow_sdk.app.actions.task_assign_action import (
    assignee_invite_body,
    ensure_task_on_hub,
    push_hub_fields,
    require_assignable_task,
)
...
async def _invite_member(email: str, child: Task, parent: Task, message: str | None = None) -> None:
    ...
    """
    # The single-assignee body plus the parent guest target — the group form is
    # the primitive's invite with one extra target, not a second wire literal.
    body = assignee_invite_body(email, child, message or f'You have been assigned the task "{parent.title}"')
    body["invitation_targets"].append({"typeid": f"task-{parent.id}", "role": HubRole.GUEST.value})
...
async def create_group_task() -> ApiResponse:
    ...
    """
    task, request_info, body, owner_email = await require_assignable_task("create-group-task")
    # Only the fan-out cares: a member task is already someone's copy, and
    # fanning it out again would nest a group under a group. A plain assign has
    # no such constraint — a sub-task is just a task.
    if task.parent_id:
        raise HTTPException(status_code=400, detail="create-group-task: a member task cannot be fanned out")
    ...
    await ensure_task_on_hub(task, request_info.someone_typeid)
    ...
    if (created or by_assignee) and _apply_group_flip(task, group.name):
        ...
        await push_hub_fields(task, ("kind", "group_name"))

...
async def handle_add_message(
    ...
    *,
    ...
    if start_settings is not None:
        ...
        for _p in (getattr(conv, "members", None) or []):
            if isinstance(_p, dict) and _p.get("user_id") and _p.get("user_id") != sender_id:
                peer_id, peer_name = _p.get("user_id"), (_p.get("name") or _p.get("email"))
                break

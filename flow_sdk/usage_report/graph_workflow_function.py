"""The daily-analysis flow's ``analyze`` GraphWorkflowFunction (subprocess runtime).

The seeded flow's ``scripts/analyze_usage.py`` is a thin shim importing
``on_graph_workflow_event`` from here, so the real logic is package-versioned and
...
def on_graph_workflow_event(event_name: str, data: dict, flow_ctx: Any) -> None:

...
def is_entity_type(segment):
    """Check if segment is a registered entity type"""
    return type_registry.get(segment) is not None
...
class APIRequest(BaseModel):
    ...
    def from_api_path(cls, api_path: str = "") -> "APIRequest":
        ...
        type_map = [is_entity_type(segment) for segment in lower_case_segments]
        ...
        typeid_pairs_bitmap = [is_entity_type(pair[0]) and is_valid_identifier(pair[1]) for pair in lower_case_pairs]
        scoped_typeid_pairs = []
        none_scoped_pairs = []
        ...
        if none_scoped_pairs:
            ...
            if target_action:
                ...
                api_request.direct_resource_type = (
                    non_scoped_segments[0] if non_scoped_segments and is_entity_type(non_scoped_segments[0]) else None
                )

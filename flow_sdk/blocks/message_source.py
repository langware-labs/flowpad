"""Process-local prompt/reply message sources.

``MessageSource`` is the smallest conversational block: one listener receives
...
class MessageRequest(DataSpec):
    """One prompt awaiting one reply from a ``MessageSource`` listener.
...
class _SimpleListener:
    ...
    def resolve(self, request: MessageRequest, text: str) -> None:
        ...
        if future is None:
            if self.closed:
                raise RuntimeError("message source listener is closed")
    ...
    def close(self, error: Exception | None = None) -> None:
        ...
        for future in self.pending.values():
            if not future.done():
                future.set_exception(error or RuntimeError("message source listener closed before replying"))
        self.pending.clear()
...
class MessageSource:
    """Factory and interface for conversational message sources.

    ``MessageSource.get("simple")`` returns a fresh process-local channel.
    """

    kind: ClassVar[str]
    _registry: ClassVar[KindRegistry[type["MessageSource"]]] = KindRegistry("message source")

    @classmethod
    def get(cls, kind: str) -> "MessageSource":
...
class _SimpleMessageSource(MessageSource):
    kind = "simple"
    ...
    async def listen(self) -> AsyncIterator[AsyncIterator[MessageRequest]]:
        if self._listener is not None:
            raise RuntimeError("message source already has an active listener")
    ...
    async def send(self, prompt: str) -> str:
        ...
        if listener is None or listener.closed:
            raise RuntimeError("message source has no active listener")
...
MessageSource._registry.register(_SimpleMessageSource)

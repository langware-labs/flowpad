"""Streaming response handler with multi-consumer queue architecture."""
...
from ..models.flow_data import FlowElementType
...
class CallbackHandler:
    """Base class for handling flow execution events."""
...
class StreamingResponseHandler(CallbackHandler):
    """Multi-consumer streaming response handler with virtual channel support."""
    ...
    async def on_user_message(self, message: str):
        from flow_sdk.core.flow.models.flow_data import FlowData, FlowElementType, FlowDataType

        flow_data = FlowData(
            flow_value=message,
            attributes={"element-type": FlowElementType.PROMPT_ECHO.value, "data-type": FlowDataType.TEXT.value},
        )
        await self.on_flow_data(flow_data)
    ...
    async def on_status(self, status: str):
        if status != self._current_status:
            ...
            flow_data = FlowData(
                flow_value=status, attributes={"element-type": FlowElementType.STATUS.value, "data-type": FlowDataType.TEXT.value}
            )
            await self.on_flow_data(flow_data)
    ...
    async def on_focus(
        ...
        if focus != self._current_focus:
            ...
            flow_data = FlowData(
                flow_value=focus,
                attributes={
                    "element-type": FlowElementType.FOCUS.value,
    ...
    async def on_shell_input(self, command: str, workdir: str):
        ...
        flow_data = FlowData(
            flow_value=command,
            attributes={
                "element-type": FlowElementType.SHELL_INPUT.value,
                "data-type": FlowDataType.TEXT.value,
                "workdir": workdir,
            },
            focus="shell",
    ...
    async def on_shell_output(self, chunk: str, channel: str):
        ...
        flow_data = FlowData(
            flow_value=chunk,
            attributes={
                "element-type": FlowElementType.SHELL_OUTPUT.value,
                "data-type": FlowDataType.TEXT.value,
                "channel": channel,
            },
            focus="shell",
    ...
    async def on_new_sources(self, sources: list[str]):
        ...
        for source in sources:
            flow_data = FlowData(
                flow_value=source, attributes={"element-type": FlowElementType.SOURCE.value, "data-type": FlowDataType.TEXT.value}
            )
            await self.on_flow_data(flow_data)
    ...
    async def on_trace(self, message: str, level: Literal["info", "warning", "error"] = "info"):
        ...
        flow_data = FlowData(
            flow_value=message,
            attributes={"element-type": FlowElementType.TRACE.value, "data-type": FlowDataType.TEXT.value, "level": level},
        )
        await self.on_flow_data(flow_data)
    ...
    async def on_error(self, error: Exception):
        ...
        flow_data = FlowData(
            flow_value=error_message, attributes={"element-type": FlowElementType.ERROR.value, "data-type": FlowDataType.TEXT.value}
        )
        await self.on_flow_data(flow_data)
    ...
    async def on_cached_message(self, cached_message: str):
        ...
        flow_data = FlowData(
            ...
            attributes={"element-type": FlowElementType.CACHED_MESSAGE.value, "data-type": FlowDataType.TEXT.value},
        )
        await self.on_flow_data(flow_data)
    ...
    async def on_state(self, key: str, data: dict):
        ...
        flow_data = FlowData(
            flow_value=data,
            attributes={"element-type": FlowElementType.STATE.value, "data-type": FlowDataType.OBJECT.value, "key": key},
        )
        await self.on_flow_data(flow_data)
    ...
    async def on_llm_end(self):
        ...
        flow_data = FlowData(
            ...
            attributes={"element-type": FlowElementType.LLM_END.value, "data-type": FlowDataType.TEXT.value},
        )
        await self.on_flow_data(flow_data)
    ...
    async def on_reasoning(self, chunk: str):
        ...
        flow_data = FlowData(
            flow_value=chunk, attributes={"element-type": FlowElementType.REASONING.value, "data-type": FlowDataType.TEXT.value}
        )
        await self.on_flow_data(flow_data)
    ...
    async def on_chat(self, chunk: str):
        ...
        flow_data = FlowData(
            flow_value=chunk, attributes={"element-type": FlowElementType.CHAT.value, "data-type": FlowDataType.TEXT.value}
        )
        await self.on_flow_data(flow_data)
    ...
    def is_same_flow_data_streaming(self, flow_data: "FlowData | None") -> bool:
        ...
        if hasattr(flow_data, "final") and flow_data.final:
            return False
        if flow_data.element_type == self.current_streaming_flow_data.element_type:
            ...
            if flow_data.element_type in FlowElementType.streamable_types():
                if hasattr(flow_data, "group_id") and flow_data.group_id:
                    if hasattr(flow_data, "channel") and flow_data.channel:
                        if (
...
class IteratorCallbackHandler(CallbackHandler):
    """Callback handler that collects responses into an async iterator."""

    def __init__(self):
        self.queue: asyncio.Queue[str | Exception | None] = asyncio.Queue()
        self._done = False
    ...
    async def on_flow_data(self, flow_data: "FlowData"):
        """Called when flow data is received."""
        pass
    ...
    async def on_ux_status(self, status: str, delay_ms: float = 1000.0):
        """Called when UX status is updated."""
        pass
    ...
    async def on_trace(self, message: str, level: Literal["info", "warning", "error"] = "info"):
        """Called when a trace message is received."""
        pass
    ...
    async def on_shell_output(self, content: str, channel: str):
        """Called when shell output is received."""
        pass
    ...
    async def iterate(self) -> AsyncGenerator[str, None]:
        """Async generator that yields chunks from the queue."""
        while True:

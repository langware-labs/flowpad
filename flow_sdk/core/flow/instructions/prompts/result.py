"""
Result instruction templates for FlowPad AI agents.
"""

from flow_sdk.builtin.artifact import artifact_descriptors
from flow_sdk.builtin.knowledge_base.knowledge_data import KnowledgeData


def _generate_artifact_table() -> str:
    """Generate artifact types table from artifact descriptors."""
    # Table header
    lines = [
    ...
    # Add each artifact type from descriptors
    for descriptor in artifact_descriptors:
        artifact_type = descriptor.artifact_type.value.lower()
        description = descriptor.description
        # Pad the artifact type to 15 characters, description to 69 characters
        lines.append(f"| {artifact_type:<15} | {description:<69} |")

    return "\n".join(lines)


def _generate_artifact_types_csv() -> str:
    """Generate CSV list of artifact types from artifact descriptors."""
    return ", ".join(f'"{descriptor.artifact_type.value.lower()}"' for descriptor in artifact_descriptors)


# Global table of allowed artifact types and descriptions
allowed_artifact_table = _generate_artifact_table()

# Global CSV list of allowed artifact types
allowed_artifact_types_csv = _generate_artifact_types_csv()


def create_result_instructions() -> KnowledgeData:
    """Create enhanced result instruction knowledge data."""
    result_text = """
...
""".strip()
    ...
    knowledge_data.add_instruction(content=result_text + "\n\n\n\n", name="result_instructions")
    return knowledge_data


# Create and export the instructions
result_instructions = create_result_instructions()

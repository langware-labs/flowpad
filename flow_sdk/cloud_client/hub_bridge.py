    ...
    async def _handle_flow_message_op(
        ...
        if op == "update":
            ...
            for field in (
                ...
                "is_read",
                "is_archived",
                # body_status / attachment_filename must sync too: the receiver
                # waits for the hub's body_status UPDATE to know the bundle is
                # downloadable (see FlowMessage.download_body).
                "body_status",
                "attachment_filename",
            ):
                if field in data:

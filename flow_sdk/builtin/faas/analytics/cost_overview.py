...
from flow_sdk.builtin.faas.analytics._pricing import (
...
def load_recent_sessions_for_cost(limit: int = 100) -> list[dict]:
    ...
    ``discover_claude_session_paths_iter`` yields in directory order, so we
    stat every path and sort by mtime DESC *before* applying ``limit`` — the
...
def _normalize_session(d: dict) -> dict:
    """Map indexer stat field names → the names the cost aggregation reads."""
    if "cache_read_tokens" not in d and "cache_read_input_tokens" in d:
        d["cache_read_tokens"] = d.get("cache_read_input_tokens")
    if "cache_creation_tokens" not in d and "cache_creation_input_tokens" in d:
        d["cache_creation_tokens"] = d.get("cache_creation_input_tokens")
    if "tool_uses" not in d:
        tu = d.get("tools_used")
        d["tool_uses"] = len(tu) if isinstance(tu, list) else (tu or 0)
    return d

...
async def _run_claude_subprocess(
    ...
    try:
        ...
        import flow_sdk as _flow_sdk_mod
        core_dir = str(Path(_flow_sdk_mod.__file__).parent / "system_assets" / "core")
        all_dirs = [core_dir] + [d for d in (context_data.get("additional_dirs") or []) if d != core_dir]
        for d in all_dirs:

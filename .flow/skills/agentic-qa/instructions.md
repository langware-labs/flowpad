...
- **Bash sandbox silently blocks `kill`.** On this harness, `kill`/`kill -9` against processes returns exit 0 but the signal never lands (process survives) unless the Bash call sets `dangerouslyDisableSandbox: true`. Machine-reclaim cleanup MUST use it. Diagnosed when 15 stale backends survived `kill -9` while in state S (interruptible) — not D-state.
- **Machine reclaim is real and necessary.** Dev box had accumulated 19 named instance_ctl instances + 15 orphaned `flow_sdk.server.run` backends (aged up to 7.6 DAYS, ppid=1) + ~40 claude workers + a stray `:9007` prod instance (its launcher `flow_sdk.server.launch 9007` RESPAWNS the backend — kill the launcher first). This drove load to 14-31. User decree: during full e2e you own the whole machine, eliminate instances on extreme load, never ask. Encoded in qa-cycle.md Step 0.0 + 0.4.
- **Sustained load floor = user GUI apps + macOS daemons** (PyCharm 125%, Chrome 80-178%, mediaanalysisd, mds_stores). NOT killable. Phase 10/timing phases get noise under it; the sweep's load-isolation + bucketing >60s as timeout is the mitigation. Don't kill GUI apps.
